...
/*$htmlHeadXtra[] = '
        <script type="text/javascript">
            $(document).ready(function() {
                $(".colorbox-inline").colorbox({inline:true, width:"800px", height:"600px"});                    
            });
        </script>
    ';  
*/
...
//	if (api_get_setting('use_session_mode')=='true' && !$nosession) {
		...
		// Request for the name of the general coach
		$sql =
		'SELECT tu.lastname, tu.firstname, ts.name, ts.date_start, ts.date_end, ts.session_category_id, ts.description 
		FROM '.$tbl_session.' ts
		LEFT JOIN '.$main_user_table .' tu
		ON ts.id_coach = tu.user_id
			...
			foreach ($category['sessions'] as $session) {
				...
				$s = get_session_title_box($session['details']['id']);
                                ...
                                if (!empty($sess_description)) {
                                    echo '<div style="background:white;padding:2px;">'.$s['description'].'</div>';
                                }
...
echo '</div>';// End content with menu
echo '</div>';// End div main right content
