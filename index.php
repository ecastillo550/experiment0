...
echo "<script type='text/javascript'>
		...
		$('#slides').mouseout(function () {
			...
			$('#slides').slides({
				preload: true,
				preloadImage: '/main/img/loadingAnimation.gif',
...
if($num_of_slides <> 0){
	...
	if($num_rows == 0){
		...
		$slides[] = array('image'=>$img_dir.'slide01.jpg','title'=>get_lang('YourTitle1'),'link'=>'#','caption'=>get_lang('YourCaption1'),'alttext'=>get_lang('AltText1'));
                $slides[] = array('image'=>$img_dir.'slide02.jpg','title'=>get_lang('YourTitle2'),'link'=>'#','caption'=>get_lang('YourCaption2'),'alttext'=>get_lang('AltText2'));
                $slides[] = array('image'=>$img_dir.'slide03.jpg','title'=>get_lang('YourTitle3'),'link'=>'#','caption'=>get_lang('YourCaption3'),'alttext'=>get_lang('AltText3'));
                $show_slide = 1;
	}
	$sql = "SELECT * FROM $slides_table WHERE language = '".Database::escape_string($language)."' ORDER BY display_order";
	$res = Database::query($sql,__FILE__,__LINE__);	
...
if ( ShoppingCartController::create()->isShoppingCartEnabled() )
{        
    $lista = EcommerceCatalog::create()->getProductList();    
    echo  $lista;
    echo '<div style="display:none;" id="shoppingMsgBody">';
...
function display_login_form() {
    ...
    api_display_language_form(false,'none;padding-left:5px');
    $form->display();
