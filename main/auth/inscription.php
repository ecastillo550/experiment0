...
$htmlHeadXtra[] = '<script type="text/javascript">
							...
							$(document).ready(function() {
								$("input[type=password]").strengthy({
									...
									validClass: "confirmation-message",
									showToggle: true,
									msgs: messages
								});
							});
...
if(file_exists($home.'register_top_'.$user_selected_language.'.html')) {
    ...
    if (!empty($open)) {
	  echo '<div class="quiz_content_actions">'.$open.'</div>';
    }
}
/****************/
...
if ($form->validate()) {
	...
	if (api_get_setting('captcha') == 'true'){
		// captcha check
		if (md5($values['captcha']) <> $_SESSION['captcha']){
			Display::display_error_message(get_lang('IncorrectCaptcha'));
			Display :: display_footer();
			exit;
		}
	}
	...
	// creating a new user
	$user_id = UserManager::create_user($values['firstname'], $values['lastname'], $values['status'], $values['email'], $values['username'], $values['pass1'], $values['official_code'], $values['language'], $values['phone'], null, null, null, 1, $usr_idrh);
	...
	if ($user_id) {
        ...
        if ($user_enrolled) { // Student was enrolled to trainings
          echo '<script>window.location.href = "'.api_get_path(WEB_PATH).'user_portal.php";</script>';
          exit;
        }
	}
