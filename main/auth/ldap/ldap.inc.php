...
function ldap_authenticate($username, $password)
{
  ...
  $user_search = ldap_get_user_search_string($username);
  //Search distinguish name of user
  $sr = ldap_search($ds, $ldap_config['base_dn'], $user_search);
  if ( !$sr ){
    error_log('LDAP ERROR : ldap_search('.$ds.', '.$ldap_config['base_dn'].", $user_search) failed");
    return false;
  }
  $entries_count = ldap_count_entries($ds,$sr);

  if ($entries_count > 1) {
    error_log('LDAP ERROR : more than one entry for that user ( ldap_search(ds, '.$ldap_config['base_dn'].", $user_search) )");
    return false;
  }
  if ($entries_count < 1) {
    error_log('LDAP ERROR :  No entry for that user ( ldap_search(ds, '.$ldap_config['base_dn'].", $user_search) )");
    return false;
  }
