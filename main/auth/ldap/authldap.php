...
/*function ldap_get_fields(){
  ...
  {
  $sr=ldap_search($ldap_connect, $ldap_search_dn, "uid=hr09580");
  }
  else
  {
  $sr=ldap_search($ldap_connect, $ldap_basedn, "uid=hr09580");
  }
  }
  else { 
  echo "cannto bind";
  ...
  echo ("test");
  $res=ldap_get_entries($ldap_connect,$sr);	
  echo "<pre>";
  print_r($res[0]);
  echo "</pre>";
  }
		...
		if ($ldap_bind_res)
		{
			if(!empty($ldap_search_dn))
			{
                               // We are using Microsoft Active directory
                                if (api_get_setting('ldap_server_type') == '1') {
                                    $sr = ldap_search($ldap_connect, $ldap_search_dn, "sAMAccountName=$login");
                                } else {
			...
			else
			{
				$sr=ldap_search($ldap_connect, $ldap_basedn, "uid=$login");
                               ...
                               // We are using Microsoft Active directory
                                if (api_get_setting('ldap_server_type') == '1') {
                                    $sr = ldap_search($ldap_connect, $ldap_basedn, "sAMAccountName=$login");
                                } else {
				  $sr = ldap_search($ldap_connect, $ldap_basedn, "uid=$login");
                                }
			}
...
function ldap_authentication_check ($uname, $passwd, $cas=false)
{
	...
	// Etablissement de la connexion anonyme avec le serveur LDAP
        $ldap_host  = ($ldap_port == 636)  ? "ldaps://".$ldap_host  : $ldap_host;
        $ldap_host2 = ($ldap_port2 == 636) ? "ldaps://".$ldap_host2 : $ldap_host2;
