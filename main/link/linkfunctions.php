...
function addlinkcategory($type)
{
    ...
    VALUES ('".Database::escape_string($urllink)."','".Database::escape_string($title)."',
   '".Database::escape_string($description)."','".Database::escape_string($selectcategory)."',
    '".Database::escape_string($order)."', '".Database::escape_string($onhomepage)."',
   '".Database::escape_string($target)."','$session_id')";
		...
		if (empty ($category_title)) {
			...
			$sql = "INSERT INTO ".$tbl_categories." (category_title, description, display_order, session_id) VALUES ('".Database::escape_string(Security::remove_XSS($category_title))."','".Database::escape_string(Security::remove_XSS($description))."', '$order', '$session_id')";
			Database::query($sql, __FILE__, __LINE__);
...
{
	global $is_allowed, $charset, $urlview, $up, $down, $_user, $linkCounter;
	$tbl_link = Database :: get_course_table(TABLE_LINK);
	...
	if($numberoflinks > 0)
	{
                echo '<div id="contentWrap"><div id="contentLeft"><ul style="min-height: 10px;" class="dragdrop nobullets category_'.$catid.'">';
                $i = 1;
		while ($myrow = Database::fetch_array($result)) {
			...
			if ($myrow['visibility'] == '1')
			{
                ...
                if (api_is_allowed_to_edit ()) {
                  echo "<td align=\"left\" valign=\"center\" width=\"6%\" class='$draggable_class'>".Display::return_icon('pixel.gif', null, array('class' => 'actionplaceholdericon actionsdraganddrop'))."</td>";                
                }else{
                  echo "<td align=\"left\" valign=\"center\" width=\"6%\" class='$draggable_class'>".Display::return_icon('pixel.gif', null, array('class' => 'placeholdericon toolshortcut_links'))."</td>";                      
                }
			...
			else
			{
				if (api_is_allowed_to_edit(null,true))
				{
					echo "<li id='recordsArray_".$myrow[0]."' class='draggable $drag'>
							<table class='data_table' width='100%'>
								<tr class='".$css_class."'>
	...
	echo '</ul></div></div>';
            ...
            echo '<div id="contentWrap"><div id="contentLeft"><ul style="min-height: 10px;" class="dragdrop nobullets category_'.$catid.'">';
            echo get_lang('ThereAreNoLinksInThisFolder');//There are no links in this folder
