...
if (isset ($_GET['search']) && $_GET['search'] == 'advanced') {
	...
	$query = "SELECT s.id, s.name, s.max_seats, s.nbr_courses, s.date_start, s.date_end, u.firstname, u.lastname , sc.name as category_name, s.visibility
			 FROM $tbl_session s
...
//filtering the session list by access_url
	...
	<!--<tr>
		...
		<tr>
		  ...
		  <!--th><a href="<php echo api_get_self(); ?>?sort=name_category<php echo $cond_url; ?>"><php echo get_lang('SessionCategoryName'); ?></a></th-->
		  <!--th><?php echo get_lang('MaximunSeats'); ?></th-->
	...
	<!--<tr class="<?php echo $i?'row_odd':'row_even'; ?>">
		  ...
		  <tr class="<?php echo $i?'row_odd':'row_even'; ?>">
			<!--  <td><input type="checkbox" id="idChecked_<?php echo $x; ?>" name="idChecked[]" value="<?php echo $enreg['id']; ?>"></td> -->
                  ...
                  <td><a href="session_course_list.php?id_session=<?php echo $enreg['id']; ?>"><?php echo $nb_courses; ?> cours</a></td>
                  <!--td><php echo api_htmlentities($enreg['category_name'],ENT_QUOTES,$charset); ?></td-->
                  <!--td><?php /*echo $enreg['max_seats'] == -1 ? '&infin;' : api_htmlentities($enreg['max_seats'],ENT_QUOTES,$charset);*/ ?></td-->
		  ...
		  <td>
			...
			<a href="<?php echo api_get_self(); ?>?sort=<?php echo $sort; ?>&action=delete&idChecked=<?php echo $enreg['id']; ?>" onclick="javascript:if(!confirm('<?php echo get_lang('ConfirmYourChoice'); ?>')) return false;"><?php echo Display::return_icon('pixel.gif', get_lang('Delete'), array('class' => 'actionplaceholdericon actiondelete')); ?></a>
		  </td>
		</tr>
