...
if($action == 'add' || $action == 'edit')
{
	...
	echo '<a href="'.api_get_self().'?category='.Security::remove_XSS($category).'">'.Display::return_icon('pixel.gif',get_lang('Back'),array('class'=>'toolactionplaceholdericon toolactionback')).get_lang("Back"); 
	if(!empty($category)){
	...
	echo '<form method="post" action="'.api_get_self().'?action='.Security::remove_XSS($action).'&category='.Security::remove_XSS($category).'&amp;id='.Security::remove_XSS($_GET['id']).'">';
...
else
{
	...
	if(!empty($category) && empty($action))
	{
		...
		echo '<a href="'.api_get_self().'?category='.$parent_id['parent_id'].'">'.Display::return_icon('pixel.gif',get_lang('Back'),array('class'=>'toolactionplaceholdericon toolactionback')).get_lang("Back");
	...
	echo '<a href="'.api_get_path(WEB_CODE_PATH).'admin/subscribe_user2course.php">'.Display::return_icon('pixel.gif',get_lang('AddUsersToACourse'),array('class' => 'toolactionplaceholdericon toolactionaddusertocourse')).get_lang('AddUsersToACourse').'</a>';
        echo '<a href="'.api_get_path(WEB_CODE_PATH).'admin/course_category.php">'.Display::return_icon('pixel.gif',get_lang('AdminCategories'),array('class' => 'toolactionplaceholdericon toolactiondocument')).get_lang('AdminCategories').'</a>';
        ...
        echo '<a href="'.api_get_path(WEB_CODE_PATH).'admin/course_user_import.php">'.Display::return_icon('pixel.gif',get_lang('ImportUsersToACourse'),array('class' => 'toolactionplaceholdericon toolactionimportcourse')).get_lang('ImportUsersToACourse').'</a>';	
