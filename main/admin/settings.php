...
$htmlHeadXtra [] = '  <script type="text/javascript" charset="utf-8">
    $(document).ready(function() {
    	// changing the course setting delegation checkboxes in a iphone style slider
      	var coursesettingdelegation = $(".coursesettingdelegationcheckbox").iphoneStyle({ checkedLabel: "' . get_lang ( 'Yes' ) . '", uncheckedLabel: "' . get_lang ( 'No' ) . '" });

      	// adding a tooltip to the course setting delegation
		var mytooltip = $(".coursesettingdelegationdiv[title]").tooltip({tip: "#tooltip", position: "bottom center", api: true});
      	
      	// when we click we have to store the value of the course setting delegation in the database
		$(".coursesettingdelegationdiv").click(function() {
			// is the checkbox checked or not
			var status = $("#"+$(this).attr("id")+" .coursesettingdelegationcheckbox").attr("checked");
			//alert(status);
			
			clicked =$(this).attr("id");
			
			// what is the variable that we just changed
			var variable = $(this).attr("id").replace("coursesettingdelegationdiv_","");
			//alert(variable);

			// storing the change
			var parameters = "variable="+variable+"&status="+status+"&action=savecoursesettingdelegation";
			$.post("'.api_get_path(WEB_PATH).'main/admin/settings_functions.php", parameters, function(theResponse){
						$("#"+clicked+" .feedback").html(theResponse);
						$("#"+clicked+" .feedback").animate({ opacity: "show" });
						setTimeout(function() {
						  $("#"+clicked+" .feedback").animate({ opacity: "hide" });
						}, 3000);
...
$htmlHeadXtra [] = '<style>
	.tooltip{ 
	    display:none; 
	    background:transparent url(' . api_get_path ( WEB_LIBRARY_PATH ) . 'javascript/images/tooltip/white_arrow_reverse.png); 
	    font-size:12px; 
	    height:70px; 
	    width:160px; 
	    padding:25px; 
...
if (!empty($_GET['category']) && !in_array($_GET['category'], array('Plugins', 'stylesheets', 'Search')))
{
	...
	foreach($settings as $row) {
		...
		if (($rowkeys['variable'] == 'course_create_active_tools' && $rowkeys['subkey'] == 'enable_search') || ($rowkeys['variable'] == 'show_tabs' && $rowkeys['subkey'] == 'my_gradebook')){
			continue;
		}
		...
		if ($row['scope'] != '-1' AND !is_null($row['scope'])) {
			...
			$form->addElement ( 'html', '<div style=" float: right; margin-bottom: 5px;   margin-right: 7px;   margin-top: 5px;" id="coursesettingdelegationdiv_' . $row ['variable'] . '" class="coursesettingdelegationdiv" title="<sub>'.get_lang('ClickToEnableOrDisable').'</sub>">
											...
											<label for="coursesettingdelegation_' . $row ['variable'] . '" style="float:left;" class="coursesettingdelegationlabel">' . get_lang ('CourseSettings') . ':&nbsp;&nbsp;</label>
		...
		switch ($row['type']) {
         ...
         if ($_GET['category'] == 'Advanced') {                            
            ...
            if (Database::num_rows($rs_install_date) > 0){
                ...
                $form->addElement('static', null, get_lang($row_install_date['comment']), get_lang('CurrentValue').' : '.date('d-m-Y H:i:s', $row_install_date['selected_value']));
            }
         }
...
//echo "\n\t<a href=\"".api_get_self()."?category=System\">".Display::return_icon($action_images['system'], api_ucfirst(get_lang('System'))).api_ucfirst(get_lang('System'))."</a>";
//echo "\n\t<a href=\"".api_get_self()."?category=Wysiwyg\">".Display::return_icon($action_images['wysiwyg'], api_ucfirst(get_lang('Wysiwyg'))).api_ucfirst(get_lang('Wysiwyg'))."</a>";
echo "\n</div>";
...
function handle_stylesheets()
{
    ...
    if ($get_status===false) {
        ...
        Display::display_error_message(get_lang('StylesheetNotHasBeenAdded'));  
    } else {
...
function upload_stylesheet($values,$picture)
{
  ...
  if($picture['type'] == 'application/zip' ) {
    ...
    $allow_extension_files=array('png', 'jpg', 'gif', 'css');
	...
	if ($_GET['action'] == 'edit') {
		...
		if(file_exists(api_get_path(SYS_PATH).'main/css/'.$css_name.'/templates.css')) {
			$template_content = str_replace('../../img/', api_get_path(REL_CODE_PATH).'img/', file_get_contents(api_get_path(SYS_PATH).'main/css/'.$css_name.'/templates.css'));
                        ...
                        file_put_contents(api_get_path(SYS_PATH).'main/css/'.$css_name.'/templates.css', $template_content);
                        //copy(api_get_path(SYS_PATH).'main/css/'.$css_name.'/templates.css', $filepath.'css/templates.css');
		}
		
                //$template_css = str_replace('images/',api_get_path(REL_CODE_PATH).'css/'.$css_name.'/images/',$template_css);
                $valcontent =  str_replace('{CSS}','<link rel="stylesheet" href="'.api_get_path(WEB_PATH).'main/css/'.$css_name.'/templates.css" type="text/css" />', $valcontent);
	...
	if( $form->validate() ) {
		...
		if ($check) {
		   ...
		   if ($_GET['action'] == 'add') {
			...
			//$position = stripos($values['template_text'], 'A:hover');                                                
                        /*
			if ($position !== false){					
                            ...
                            $content_template = '<head><style type="text/css">.text{font-weight: normal;}</style></head><body><!-- white table for the course -->'.Database::escape_string($real_content_template).'<!-- end white table for the course --></body>';
			}*/
                        $content_template = '<head>{CSS}</head><body><!-- white table for the course -->'.Database::escape_string($real_content_template).'<!-- end white table for the course --></body>';
