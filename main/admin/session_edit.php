...
$result=Database::query("SELECT name,max_seats,cost,description,date_start,date_end,id_coach, session_admin_id, nb_days_access_before_beginning, nb_days_access_after_end, session_category_id, visibility, certif_template, certif_tool, certif_min_score, certif_min_progress FROM $tbl_session WHERE id='$id'",__FILE__,__LINE__);
...
if ($_POST['formSent']) {
	...
	$return = SessionManager::edit_session($id,$name,$description,$year_start,$month_start,$day_start,$year_end,$month_end,$day_end,$nolimit, $id_coach, $id_session_category, $cost, $certif_template, $certif_tool, $certif_min_score, $certif_min_progress);
...
<tr>
  <td width="40%"><?php echo get_lang('Certificate') ?></td>
  <td width="60%">
    <?php
        ...
        if (!empty($certificates)) {
            ...
            foreach ($certificates as $tpl_id => $certificate) {
                echo '<option value="'.$tpl_id.'" '.($formSent && $tpl_id == $certif_template?'selected="selected"':($infos['certif_template'] == $tpl_id?'selected="selected"':'')).'>'.$certificate['title'].'</option>';
            }
            echo '</select>';
        }        
    ?>
...
<tr>
  <td width="40%"><div class="certificate-tool session-certificate-score"><?php echo get_lang('CertificateTool'); ?></div></td>
  <td width="60%">
      <div class="certificate-tool session-certificate-score">
        <input type="radio" name="certificate_tool" class="certificate-tool-radio" id="certificate-tool-radio1" value="module" <?php echo ($formSent && $certif_tool == 'module'?'checked=checked':($infos['certif_tool'] == 'module'?'checked="checked"':'') )?> />&nbsp;<?php echo get_lang('Modules'); ?>&nbsp;&nbsp;
...
<tr>
  <td width="40%"><div class="certificate-module-eval-type session-certificate-score"><?php echo get_lang('CertifEvaluationType'); ?></div></td>
  <td width="60%">
      <div class="certificate-module-eval-type session-certificate-score">
        <input type="radio" name="certif_evaluation_type" class="certificate-module-radio" id="certificate-module-radio1" value="1" <?php echo (isset($_POST['certif_evaluation_type']) && $_POST['certif_evaluation_type'] == 1?'checked=checked':($infos['certif_min_progress'] != 0.00?'checked="checked"':'') )?> />&nbsp;<?php echo get_lang('Progress'); ?>&nbsp;&nbsp;
        <input type="radio" name="certif_evaluation_type" class="certificate-module-radio" id="certificate-module-radio2" value="2" <?php echo (isset($_POST['certif_evaluation_type']) && $_POST['certif_evaluation_type'] == 2?'checked=checked':($infos['certif_min_score'] != 0.00?'checked="checked"':'') )?> />&nbsp;<?php echo get_lang('Score'); ?>
      </div>
  </td>
</tr>

<tr>
  <td width="40%"><div class="certificate-score session-certificate-score"><?php echo get_lang('CertificateMinEvaluation').' (%)'; ?></div></td>
  <td width="60%">
      <div class="certificate-score session-certificate-score">
        <input type="text" name="certificate_min_score" id="certificate-score-input" value="<?php echo (isset($_POST['certificate_min_score'])?$_POST['certificate_min_score']:($infos['certif_min_progress'] != 0.00?$infos['certif_min_progress']:$infos['certif_min_score']))?>" />
      </div>
  </td>
</tr>
<?php endif; ?>
