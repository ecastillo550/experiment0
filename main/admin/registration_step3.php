...
if (isset($_POST['submit_plus'])) {
	echo '<script>window.location.href = "registration.php?iden='.$_SESSION['iden'].'&wish='.$_SESSION['wish'].'&id='.intval($_SESSION['cat_id']).'&prev=2";</script>';	
}
if( $form->validate()) {
    ...
    if (isset($user['submit'])) {
        // go to step 3b
        if ($iden == 1 || ($iden == 0 && $wish == 1)) {
            echo '<script>window.location.href = "registration_step3b.php?iden='.$iden.'&wish='.$wish.'&id='.$id.'&next=3b";</script>';
        } else {
            if (isset($_SESSION['payer_info'])) {
                unset($_SESSION['payer_info']);
            }
            // go to step 4
            echo '<script>window.location.href = "feedback.php?iden='.$iden.'&wish='.$wish.'&id='.$id.'&next=4";</script>';
        }
    }
}
$form->display();
