				...
				{					
					...
					{						 
						...
						//	header('Location: document.php?curdirpath='.urlencode($dir));
							...
							{
							echo "<script>window.location.href='showinframes.php?".api_get_cidReq()."&curdirpath=".urlencode($dir)."&file=".$dir.'/'.$filename.".".$extension."';</script>";
							}
							else
							{							
...
if( isset($info_message))
{
    ...
    echo '<div class="normal-message new-message">'.$info_message.'</div>';
	if (isset($_POST['origin']))
	{
		$slide_id=$_POST['origin_opt'];
		//nav_to_slideshow($slide_id);
