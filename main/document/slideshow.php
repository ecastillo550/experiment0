...
if (isset($_GET['msg']) ) {		
	if($_GET['msg'] == 'DEL') {
      if($_GET['msg'] == 'DEL')   echo '<div class="normal-message new-message">'.get_lang('DocDeleted').'</div>';
	...
	/*	echo '<div class="section">';
		...
		echo '<div class="sectionfooter" style="text-align:center;">';	
			...
			// delete
				...
				echo '<a style="'.$img_style.'" href="document.php?'.api_get_cidreq().'&curdirpath='.$pathurl.'&delete='.urlencode($row['path']).'&slide_id='.$slide_id;
				if($new_id!=''){ 
				   echo '&new_id='.$new_id;
				}
				echo '" onclick="return confirmation(\''.basename($path).'\');">'.Display::return_icon('pixel.gif','',array('class'=>'actionplaceholdericon actiondelete')).'</a>';
