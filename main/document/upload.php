...
if ($upload_ok) {
  ...
  if(in_array($extension, array('wmv','mpg','mpeg','mov','avi')) && api_get_setting('search_enabled')=='true') {
   ...
   } else { // The uploaded file is not a movie then ...   
    //file got on the server without problems, now process it
    $new_path = handle_multiple_uploaded_document($_course, $_FILES['user_upload'], $i, $base_work_dir, $_POST['curdirpath'], $_user['user_id'], $to_group_id, $to_user_id, $max_filled_space, $_POST['unzip'], $_POST['if_exists']);
