...
function display_announcement($id=''){
  ...
  $attachment_list = AnnouncementManager::get_attachment($_GET['ann_id']);
	...
	if(api_is_allowed_to_edit()){
		...
		echo  '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
		echo  '<a href="'.api_get_self().'?'.api_get_cidreq().'&action=delete&id='.$announcement['id'].'">'.Display::return_icon('pixel.gif',get_lang('DeleteAnnouncement'),array('align' => 'absmiddle','class'=>'actionplaceholdericon actiondelete')).' '.get_lang('DeleteAnnouncement').'</a>';
		echo  '</div>';
	}
