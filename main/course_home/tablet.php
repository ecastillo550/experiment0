...
?>
	<div class="courseadminview" style="border:0px; margin-top: 0px;padding:5px 0px;">
		...
		<?php			
			echo '<img src="'.$server_protocol.$current_host.'/'.$path_work.'main/img/navigation/ajax-loader-grey.gif"/>'."&nbsp;&nbsp;";
			echo get_lang('PleaseStandBy');
		?>
		</div>
		<div class="confirmation-message rounded" id="id_confirmation_message" style="display:none"></div>
	</div>
	...
	?>
	<span class="sectiontablettitle"><?php echo get_lang("Authoring") ?></span>
	<div class="sectiontablet main_activity">
		<table>
			<?php $my_list = get_tools_category(TOOL_AUTHORING); show_tools_category($my_list);?>
		</table>
	</div>
	<span class="sectiontablettitle"><?php echo get_lang("Interaction") ?></span>
	<div class="sectiontablet main_activity">
		<table>
			<?php $my_list = get_tools_category(TOOL_INTERACTION); show_tools_category($my_list);?>
		</table>
 	</div>
	<span class="sectiontablettitle"><?php echo get_lang("Administration") ?></span>
	<div class="sectiontablet main_activity">
		<table>
	...
	?>
		<div class="section main_activity activity_student_view">
			<table>
				...
				if (count($my_list) == 0) {
					echo '<div align="center"><br/><br/><br/><br/><br/><strong>'.get_lang('ThereAreNotVisibleToolsNow').'<strong></div>';
				} else {
