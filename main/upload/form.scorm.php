...
$form -> addElement ('style_submit_button', 'convert', get_lang('Send'), array('style'=>"margin-bottom:20px",'class'=>'save'));
$form -> addElement ('html','<br/><br/>');
$form->addElement('html', '<h3 class="title">'.get_lang('UploadedFtpFiles').'</h3>');
$list = get_zip_files_in_garbage();
$select_file_name = &$form->addElement('select','file_name','<div style="text-align:right;width:125px;">'.get_lang('UploadLocalFileFromGarbageDir').'</div>', null, array('style' => 'text-align:right;width:220px'));
foreach($list as $file){
	$select_file_name->addOption($file,$file);
}
$form -> addElement ('style_submit_button', 'submit', get_lang('Validate'), array('style'=>"",'class'=>'upgrade_link'));
...
echo '<div id="content" class="rel">';
  ...
  if (isset($_SESSION['is_in_serious_game']) && $_SESSION['is_in_serious_game'] == 0) {
   ...
   echo Display::return_icon("avatars/interaction.png", get_lang('Build'), array(	'class'	=> "abs", 'style'	=> "margin:50px 0 0 50px; top:0; left:0;"));
			...
			echo '<div class="abs" style="margin:0 50px 50px 0; right:0; top:25%; ">';
				$form -> display();
			echo '</div>';
		echo '</div>';
