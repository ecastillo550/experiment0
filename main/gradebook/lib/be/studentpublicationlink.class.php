...
// FUNCTIONS IMPLEMENTING ABSTRACTLINK
	...
	public function get_view_url ($stud_id) {
		...
		$sql = 'SELECT pub.id'
				...
				.' AND prop.insert_user_id = '.Database::escape_string($stud_id)
				.' AND prop.ref = pub.id'
		...
		if ($fileurl = Database::fetch_row($result)) {
			...
			$url = api_get_path(WEB_PATH)
					.'main/gradebook/open_document.php?coursecode='.$this->get_course_code().'&amp;coursedir='
					.$course_info['directory']
					.'item=studentpublication&amp;itemid'
					.$fileurl[0].'&amp;user_id='.$stud_id;
...
// FUNCTIONS IMPLEMENTING ABSTRACTLINK
    ...
    public function get_all_links() {
		...
		$sql = 'SELECT id,url FROM '.$tbl_grade_links.' WHERE filetype="folder"';
		$result = Database::query($sql, __FILE__, __LINE__);
