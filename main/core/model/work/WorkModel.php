...
class WorkModel 
{
	...
	public function get_assignment_data() {
		...
		$sql = "SELECT work.id AS assignment_id,work.title,work.filetype, work.description,work.url,work.parent_id,work.qualification,workassign.ends_on 
					FROM {$this->tableAssignment} work LEFT JOIN $TSTDPUBASG workassign ON work.id = workassign.publication_id
		...
		while ($assignment = Database::fetch_array($res)) {		
			...
			$sql_paper = "SELECT * FROM {$this->tableAssignment} work,$itemproperty_table ip WHERE work.id = ip.ref AND ip.insert_user_id IN (".$users_list.") AND work.parent_id = ".$assignment_id." AND work.filetype = 'file'";
			}
			else {
			...
			if(api_is_grouptutor($_course,api_get_session_id(),api_get_user_id())){
			$users_list = $this->get_grouptutor_users(api_get_user_id());
			$sql_corrected = "SELECT * FROM {$this->tableAssignment} work,$itemproperty_table ip WHERE work.id = ip.ref AND ip.insert_user_id IN (".$users_list.") AND parent_id = ".$assignment_id." AND qualificator_id = 1 AND filetype = 'file'";
			}
			else {
...
//				$table->set_header(8, get_lang('DownloadAll'));
     ...
     *  @return`    array   Assignments
     */
    public function getDisplayActionIcons($assignmentId) {
		...
		if(isset($_REQUEST['action']) && ($_REQUEST['action'] == 'correct_paper' && $_REQUEST['corrected'] != 'Y')){
			...
			$action_icons .= '<a href="'.api_get_self().'?'.api_get_cidreq().'&action=view_papers&assignment_id='.$assignmentId.'&origin='.$origin.'&gradebook='.$gradebook.'&curdirpath='.$parent_dir.'">'.Display::return_icon('pixel.gif',get_lang('BackToPapersList'),array('class' => 'toolactionplaceholdericon toolactionback')).get_lang('BackToPapersList').'</a>';
		}
		else if(isset($_REQUEST['action']) && $_REQUEST['action'] != 'delete' && $_REQUEST['done'] != 'Y'){
			$action_icons .= '<a href="'.api_get_self().'?'.api_get_cidreq().'&origin='.$origin.'&gradebook='.$gradebook.'&curdirpath='.$parent_dir.'">'.Display::return_icon('pixel.gif',get_lang('BackToWorksList'),array('class' => 'toolactionplaceholdericon toolactionback')).get_lang('BackToWorksList').'</a>';
		}

		if(api_is_platform_admin() || api_is_grouptutor($_course,api_get_session_id(),api_get_user_id()) || api_is_allowed_to_edit()){
			$action_icons .=	'<a href="'.api_get_self().'?'.api_get_cidreq().'&action=add&toolgroup='.Security::remove_XSS($_GET['toolgroup']).'&amp;createdir=1&origin='.$origin.'&gradebook='.$gradebook.'">'.Display::return_icon('pixel.gif',get_lang('CreateAssignment'),array('class' => 'toolactionplaceholdericon toolactionnewassignment')).get_lang('CreateAssignment').' </a>';
		}
...
//aux_id      
	...
	public function getForm($cur_dir_path) {		
		...
		if (isset ($_REQUEST['createdir']) && (api_is_allowed_to_edit() || api_is_grouptutor($_course,api_get_session_id(),api_get_user_id()))) {
			$form = new FormValidator('new_assignment', 'post','index.php?action=add&createdir='.$_REQUEST['createdir'].'&done=Y&origin='.api_get_tools_lists($_REQUEST['origin']).'&gradebook='.(empty($_GET['gradebook'])?'':'view'). $add_lp_param);
			$form->addElement('header', '', get_lang('CreateAssignment'));
		}
		elseif($this->assignment_id) {			
			$form = new FormValidator('edit_assignment', 'post','index.php?action=edit&edit_dir='.$_REQUEST['edit_dir'].'&assignment_id='.$this->assignment_id.'&done=Y&origin='.api_get_tools_lists($_REQUEST['origin']).'&gradebook='.(empty($_GET['gradebook'])?'':'view'). $add_lp_param);
		...
		//	$form->addElement('textarea','description',get_lang('Description'),array ('rows' => '3', 'cols' => '62'));
			...
			$defaults['score'] = 20;
			$defaults['deadline'] = date('Y-m-d 12:00:00', strtotime('+1 month'));
			}
		...
		elseif ($ctok==$assignment['sec_token']) {	
			...
			if (!empty($created_dir)) {
				...
				$sql_add_publication = "INSERT INTO {$this->tableAssignment} SET " .
					...
					$dateparts = explode("-", $datetime[0]);		
					$deadline = $dateparts[1].'/'.$dateparts[2].'/'.$dateparts[0];
					$this->send_mail_to_students($new_dir,$description,$deadline);					
			}		
		}
	}
	...
	public function save_correct_paper() {
		...
		if (!empty($_FILES['file']['size'])) {
                    ...
                    if (!empty($extension)) { $new_file_name .= '.'.$extension; }                    
	...
	public function moveto($move_file, $moveurl_to) {
		...
		if ($path = $this->get_work_path($move_file)) {
			if (move(api_get_path(SYS_COURSE_PATH) . $_course['path'] . '/' . $path, $base_work_dir . '/' . $move_to)) {
				...
				$message = '<div class="error-message rounded">'.get_lang('Impossible').'</div>';
			}
		} else {
				$message = '<div class="error-message rounded">'.get_lang('Impossible').'</div>';
		}   
