...
function show_agenda_form($input_values, $date='', $dialog = false) {
	...
	// initiate the object
	$form = new FormValidator ( 'new_agenda_item', 'post', $_SERVER ['REQUEST_URI'].'&op=new&view='.intval($_GET['view']) );
        if (!$dialog) {
	...
	if ($form->validate ()) {
		...
		if (isset ( $_POST ['submit_agenda_item'] ) || isset($form_actions['submit_agenda_item'])) {
			...
			//echo calendar_javascript();
                ...
                echo '<script>location.href="'.api_get_path(WEB_CODE_PATH).'calendar/agenda.php?'.api_get_cidreq().$calendar_view.'";</script>';
	} else {
 ...
 * @version june 2010
 */
function show_myagenda_form($input_values, $date='', $dialog = false) {
	// initiate the object
	...
	$form->addElement ( 'html', '</div>' );
        ...
        $form->addElement ( 'html', '<div id="action-bottom">');
	// The OK button
        if ((isset($_GET['action']) && $_GET['action'] == 'myedit') && $dialog) {
            $form->addElement ('style_button', 'delete_myagenda_item', '', 'class="save" value="'.get_lang('Delete').'" id="delete-myagenda-item"');               
        }
        $form->addElement ( 'style_submit_button', 'submit_agenda_item', get_lang ( 'SaveEvent' ) ,'class="save"');                
        $form->addElement ( 'html', '</div>' );
	...
	if ($form->validate ()) {
		$values = $form->exportValues ();
                ...
                if (isset ( $_POST ['submit_agenda_item'] )) {
                        ...
                        Display::display_confirmation_message ( get_lang ( 'AgendaStored' ) );
                        echo '<script>location.href="'.api_get_path(WEB_CODE_PATH).'calendar/myagenda.php?'.$calendar_view.'";</script>';
                }
                /*} else {
                    echo '<script>location.href="'.api_get_path(WEB_CODE_PATH).'calendar/myagenda.php?'.$calendar_view.'";</script>';
                }*/
 ...
 * @version june 2010
 */
function show_platformagenda_form($input_values, $date='', $dialog = false) {
	// initiate the object
	$form = new FormValidator ( 'new_myagenda_item', 'post', $_SERVER ['REQUEST_URI'] );
	...
	if ($form->validate ()) {
		...
		if (isset ( $_POST ['submit_agenda_item'] )) {
			...
			//Display::display_confirmation_message (get_lang( 'AgendaStored' ) );
                        echo '<script>location.href="'.api_get_path(WEB_CODE_PATH).'admin/agenda.php?'.$calendar_view.'";</script>';
		}
...
function delete_myagenda_item($id) {
	...
	// Database table definition
	$tbl_agenda = Database :: get_user_personal_table(TABLE_PERSONAL_AGENDA);
	Database::query("DELETE FROM $tbl_agenda WHERE id = ".intval($id)." AND user = ".intval($_user['user_id']));
...
function get_course_agenda_items($course_db='', $coursecode, $full_information=false,$editable=true){
	...
	while ($row=Database::fetch_array($result))
	{
		...
		{                                            
                    $events[$row['id']] = array('id'=>'course_events'.$row['id'], 'title'=>htmlentities($row['title']), 'start'=>date('c',$row['start']), 'end'=>date('c',$row['end']), "allDay"=>false, 'visibility'=>$row['visibility'],'className'=>$class_name, 'type'=>'course', 'editable'=>$editable, 'content'=>$row['content']);
		}
	}
...
function get_session_events($coursecode){
	...
	while ($row=Database::fetch_array($result)) {
		//$events[] = array('id'=>'session'.$row['id'], 'title'=>htmlentities($row['name']), 'start'=>date('c',$row['date_start']), 'end'=>date('c',$row['date_end']), "allDay"=>false, 'visibility'=>'1','className'=>'session', 'editable' => false);
                $events[] = array('id'=>'session'.$row['id'], 'title'=>htmlentities($row['name']), 'start'=>date('c',$row['date_start']), 'end'=>date('c',$row['date_end']), "allDay"=>false, 'visibility'=>'1','className'=>'session', 'editable' => false, 'content'=>$row['content']);
	}                
	return $events;
}
...
function get_quiz_events($coursedb){
	...
	while ($row=Database::fetch_array($result))
	{
		//$events[] = array('id'=>'quiz'.$row['id'], 'title'=>htmlentities($row['title']), 'start'=>date('c',$row['date_start']), 'end'=>date('c',$row['date_end']), "allDay"=>false, 'visibility'=>$row['visiblity'],'className'=>'quiz', 'editable' => false);
                $events[] = array('id'=>'quiz'.$row['id'], 'title'=>htmlentities($row['title']), 'start'=>date('c',$row['date_start']), 'end'=>date('c',$row['date_end']), "allDay"=>false, 'visibility'=>$row['visiblity'],'className'=>'quiz', 'editable' => false, 'content'=>$row['content']);
	}
...
function get_assignment_events($coursedb)
{
	...
	while ($row=Database::fetch_array($result))
	{
		//$events[] = array('id'=>'assignment'.$row['id'], 'title'=>htmlentities(str_replace('/','',$row['title'])), 'start'=>date('c',$row['date_start']), 'end'=>date('c',$row['date_end']), "allDay"=>false, 'visibility'=>'1','className'=>'assignment', 'editable' => false);
                $events[] = array('id'=>'assignment'.$row['id'], 'title'=>htmlentities(str_replace('/','',$row['title'])), 'start'=>date('c',$row['date_start']), 'end'=>date('c',$row['date_end']), "allDay"=>false, 'visibility'=>'1','className'=>'assignment', 'editable' => false, 'content'=>$row['content']);
	}
	//return array();
	return $events;
}
	...
	while ($row=mysql_fetch_assoc($result))
	{
		...
		else
		{
			//$events[] = array('id'=>'platform'.$row['id'], 'title'=>htmlentities($row['title']), 'start'=>date('c',$row['start']), 'end'=>date('c',$row['end']), "allDay"=>false, 'visibility'=>'1','className'=>'platform', 'editable'=>$editable);
                        $events[] = array('id'=>'platform'.$row['id'], 'title'=>htmlentities($row['title']), 'start'=>date('c',$row['start']), 'end'=>date('c',$row['end']), "allDay"=>false, 'visibility'=>'1','className'=>'platform', 'editable'=>$editable, 'content'=>$row['content']);
	}
	}
	return $events;
}
...
function get_personal_agenda_items($full_information=false) {
	...
	while ($row=mysql_fetch_assoc($result))
	{
		...
		{
			$events[$row['id']] = array('id'=>'personal'.$row['id'], 'title'=>htmlentities($row['title']), 'start'=>date('c',$row['start']), 'end'=>date('c',$row['end']), "allDay"=>false, 'visibility'=>'1','className'=>'personal', 'parent_event_id' => isset($row['parent_event_id'])?intval($row['parent_event_id']):0, 'editable' => !empty($row['parent_event_id'])?false:true, 'content'=>$row['text']);
		}
	}
	return $events;
}
					...
					$.ajax({
					  ...
					  data: {action: 'resize', id: event.id, daydelta: dayDelta, minutedelta: minuteDelta}
                                ...
                                eventClick: function(event,jsEvent,view){
                                    ...
                                    var agenda_id = parseInt(event.id.replace('course_events', ''));
                                    
                                    if (isNaN(agenda_id)) {
                                        var agenda_id = parseInt(event.id.replace('session', ''));
                                    }
                                    ...
                                    $('#dialog-course-event-edit-form').dialog({modal: true, title: '".get_lang('AgendaEdit')."', width: '600px'});
                                    return false;
                                }
			});
	...
	return "
		...
		$(document).ready(function() {
			...
			var DokeosCalendar = $('#calendar').fullCalendar({
				...
				eventRender: function(calEvent, element) {
					// add the id to the rendered element so that we can use this for the detail
                    var sess = $('#'+calEvent.id).html();
                    if (typeof sess === 'undefined' || !sess) {
                      element.attr('id',calEvent.id);
                    }else
                    {
                      element.attr('id',calEvent.id+'del');
                    }
                ...
                eventAfterRender: function(calEvent, element) {
                    if(calEvent.className[0] == 'session') {
                      element.css('height','auto');
                      var sess = $('#'+calEvent.id+'del').html();
                      if (typeof sess !== 'undefined' || sess) {
                        $('#'+calEvent.id+'del').remove();
                      }
                    }
                },
				eventDrop: function(event,dayDelta,minuteDelta,allDay,revertFunc) {
                ...
                eventClick: function(event, jsEvent, view){
                    var start_date_value = $.datepicker.formatDate('D d M yy', event.start);
                    var end_date_value  = $.datepicker.formatDate('D d M yy', event.end);
                    var agenda_id = parseInt(event.id.replace('personal', ''));
                    if (isNaN(agenda_id)) {
                      var agenda_id = parseInt(event.id.replace('session', ''));
                    }
                    ...
                    $('#dialog-myevent-edit-form').dialog({modal: true, title: '".get_lang('AgendaEdit')."', width: '600px'});
                  }
			});
...
function display_dialog_platform_event_form() {
    ...
    $_GET['action'] = 'add';
    show_platformagenda_form(array(), '', true);
    echo '</div>';            
}
 ...
 */
function calendar_goto($type = 'personal') {
    $curr_view = '';
    ...
    switch ($type) {
        case 'personal':
            echo '<script>location.href="'.api_get_path(WEB_CODE_PATH).'calendar/myagenda.php?view='.$curr_view.'";</script>';
            break;
        case 'platform':
            echo '<script>location.href="'.api_get_path(WEB_CODE_PATH).'admin/agenda.php?view='.$curr_view.'";</script>';
            break;
        default:
            echo '<script>location.href="'.api_get_path(WEB_CODE_PATH).'calendar/agenda.php?view='.$curr_view.'";</script>';
            break;    
    }    
    return false;
}
