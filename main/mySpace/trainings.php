...
$isDisplayUserOverview = ($_GET['display'] == 'useroverview');
$isDisplayCoaches = ($_GET['display'] == 'coaches');
...
if (!$export_csv) {
	...
	if ($_GET['view'] == 'admin' AND $isDisplayUserOverview) {
	...
	if ($export_csv) {
		...
		echo '
		 <div class="report_section">
			<h4>
				<a href="student.php">'.Display::return_icon('pixel.gif',get_lang('Probationers'),array('class'=>'actionplaceholdericon actionstudentstutorview')).'&nbsp;'.get_lang('Probationers').' ('.$nb_students.')'.'</a>
			</h4>
			<table id="data_table_trainings">
				<tr>
					<td>
...
if ($view == 'coach') {
	...
	if ($export_csv) {
		...
		echo '
		 <div class="report_section">
			<h4>
				<a href="session.php">'.Display::return_icon('pixel.gif',get_lang('Sessions'),array('class'=>'actionplaceholdericon actionsessionstutorview')).'&nbsp;'.get_lang('Sessions').' ('.$nb_sessions.')'.'</a>
			</h4>
			<table id="data_table_trainings">
				<tr>
					<td>
...
if (api_is_allowed_to_create_course() && $view == 'teacher') {
	...
	if ($nb_teacher_courses) {
		...
		$table -> set_header(2, get_lang('AvgTimeSpentInTheCourse').Display :: return_icon('info3.gif', get_lang('TimeOfActiveByTraining'), array('align' => 'absmiddle', 'hspace' => '3px', 'class'=>"print_invisible")), false);
		$table -> set_header(3, get_lang('AvgStudentsProgress').Display :: return_icon('info3.gif', get_lang('AvgAllUsersInAllCourses'), array('align' => 'absmiddle', 'hspace' => '3px', 'class'=>"print_invisible")), false);
		$table -> set_header(4, get_lang('AvgCourseScore').Display :: return_icon('info3.gif', get_lang('AvgAllUsersInAllCourses'), array('align' => 'absmiddle', 'hspace' => '3px', 'class'=>"print_invisible")), false);
		$table -> set_header(5, get_lang('AvgExercisesScore').Display :: return_icon('info3.gif', get_lang('AvgAllUsersInAllCourses'), array('align' => 'absmiddle', 'hspace' => '3px', 'class'=>"print_invisible")), false);
...
{
	// to hide useless title in print css
	$displayCoachesClass = ($isDisplayUserOverview)	? "print_invisible" : "";
	$displayUserOverviewClass = ($isDisplayCoaches)	? "print_invisible" : "";
	
	echo '<a href="'.api_get_self().'?view=admin&amp;display=coaches" class="'.$displayCoachesClass.'">'.get_lang('DisplayCoaches').'</a> | ';
	echo '<a href="'.api_get_self().'?view=admin&amp;display=useroverview" class="'.$displayUserOverviewClass.'">'.get_lang('DisplayUserOverview').'</a>';
	if ($isDisplayUserOverview) {
		echo ' | <a href="'.api_get_self().'?view=admin&amp;display=useroverview&amp;export=options" class="print_invisible">'.get_lang('ExportUserOverviewOptions').'</a>';
