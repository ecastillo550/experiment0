...
if (!empty ($_GET['student'])) {
	...
	if($_user['status']==DRH && $info_user['hr_dept_id']!=$_user['user_id'] && !api_is_platform_admin()) {
		api_not_allowed();
	}
	...
	if(!empty($course_id))
		$first_connection_date = Tracking :: get_first_connection_date_on_the_course($info_user['user_id'], $course_id);
	else
	...
	if(!empty($course_id))
		$last_connection_date = Tracking :: get_last_connection_date_on_the_course($info_user['user_id'], $course_id);
	else
...
<?php
	...
	'style="float:' . ($text_dir == 'rtl' ? 'left' : 'right') . '; padding:5px;margin-right:10px;" ';
		...
		if (Database :: result($result_visibility_quizz, 0, 'visibility') == 1) {
			...
			if (Database :: num_rows($result_exercices) > 0) {
				while ($exercices = Database :: fetch_array($result_exercices)) {
					...
					echo "	</td><td align='center'>$exe_date</td>
													<td align='center'>
												 ";
					echo $essais['essais'];
					echo "	</td>
					...
					$sql_last_attempt = 'SELECT exe_id FROM ' . $tbl_stats_exercices . ' WHERE exe_exo_id="' . $exercices['id'] . '" AND exe_user_id="' . Security :: remove_XSS($_GET['student']) . '" AND exe_cours_id="' . $info_course['code'] . '" AND orig_lp_id = 0 AND orig_lp_item_id = 0 AND status <> "incomplete" ORDER BY exe_date DESC LIMIT 1';
