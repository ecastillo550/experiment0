			...
			if (isset($query) && $query!='') {		
				...
				if (is_array($users) && count($users)> 0) {		
					foreach($users as $user) {
						...
						$link = '<a href="'.api_get_path(WEB_PATH).'main/messages/send_message_to_userfriend.inc.php?view_panel=2&height=260&width=610&user_friend='.$user['user_id'].'" class="thickbox" title="'.get_lang('SendInvitation').'">'.Display :: return_icon('pixel.gif', get_lang('SocialInvitationToFriends'),array('class'=>'actionplaceholdericon actioninvitejoinfriends')).'&nbsp;'.get_lang('SendInvitation').'</a>';
				...
				if (is_array($groups) && count($groups)>0) {
					...
					foreach($groups as $group) {
						...
						$grid_item_1 = '';
						$item_1 = '<div>'.$url_open.$group['picture_uri'].'<p class="social-groups-text1"><strong>'.$name.'<br />('.$count_users_group.')</strong></p>'.$url_close.'</div>';
