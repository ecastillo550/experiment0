...
case COURSEMANAGER :
    if (api_is_platform_admin()) {
            ...
            if(isset($_POST['search_name_q'])){ 
                $sql2 = 'SELECT user_id,lastname,firstname,username FROM '.$tbl_my_user.' u WHERE status <> 6 AND user_id<>"'. Database::escape_string($user_id).'" AND (lastname like "%'.$search_name.'%" OR firstname like "%'.$search_name.'%")';
                //$sql2 = $format; 
            }
            else{
                $sql2 = 'SELECT user_id,lastname,firstname,username FROM '.$tbl_my_user.' u WHERE status <> 6 AND user_id<>"'. Database::escape_string($user_id).'" ';  
            }
