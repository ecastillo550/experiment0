...
switch ($current_page) {
    case 'new_post' :
        ...
        Display :: display_tool_header($nameTools, 'Blogs');
        break;
    case 'manage_tasks' :
...
//Display::display_tool_header($nameTools,'Blogs');
?>
<div class="quiz_content_actions"><table width="100%"><tr><td class="blog_fontcolor"><?php echo Blog::get_blog_title($blog_id); ?></td></tr>
        <tr><td ><?php echo Blog::get_blog_subtitle($blog_id); ?></td></tr></table></div>

<table width="940px">
    <tr>
        <td width="206px" class="blog_left" valign="top">
                <?php
            ...
            <table width="100%" >
                <tr>
                    <td><div class="blog_fontcolor"><?php echo get_lang('Search'); ?></div></td>
                </tr>
                <tr>
                    <td class="blog_menu">
                        <form action="blog.php" method="get" enctype="multipart/form-data">
                            ...
                            <input type="text" size="20" name="q" value="<?php echo (isset($_GET['q']) ? $_GET['q'] : ''); ?>" />
                    </td>
                </tr>
                <tr><td>
            ...
            <table width="100%">
                <tr>
                    <td><div class="blog_fontcolor"><?php echo get_lang('MyTasks') ?></div></td>
                </tr>
                <tr>
                    <td class="blog_menu">
        ...
        <td valign="top" class="blog_right" width="700px">

<?php
if (!empty($current_page) && !isset($_GET['month'])) { //&& !isset($_GET['task_id']) && isset($_GET['filter']
...
switch ($current_page) {
    ...
    case 'execute_task' :
        if (isset($_GET['post_id']))
            ...
            if (isset($_GET['filter']) && !empty($_GET['filter'])) {
                Blog :: display_select_task_by_date($blog_id, Database::escape_string($_GET['filter']));
            }
            else {
                Blog :: display_select_task_by_id($blog_id,$task_id);
            }
...
if (!empty($current_page) && !isset($_GET['month'])) { //&& !isset($_GET['task_id'])
    echo '</div>';
