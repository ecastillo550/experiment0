...
<div id="main">
	<div id="generic_tool_header">
		...
		<?php if (strcmp($tool_name,'Chat') == 0) $target = '_parent'; else $target = '_self';?>
                        <?php if ($_cid == -1) {?>
                            <a href="<?php echo api_get_path(WEB_PATH); ?>index.php<?php echo $param; ?>" id="back2home" target="<?php echo $target ?>"><img src="<?php echo api_get_path(WEB_IMG_PATH);?>spacer.gif" width="42px" height="37px" /></a>
                        <?php } else {
                            $course_path = !empty($_course['path']) ? $_course['path'] : $_course['directory'];
                            ?>
                            <a href="<?php echo api_get_path(WEB_COURSE_PATH).$course_path; ?>/index.php<?php echo $param; ?>" id="back2home" target="<?php echo $target ?>"><img src="<?php echo api_get_path(WEB_IMG_PATH);?>spacer.gif" width="42px" height="37px" /></a>
                        <?php }?>
		</div>
		<?php
			// name of training
