...
//$tmp_check_content = strip_tags($intro_content);
$tmp_check_content = $intro_content;
if(trim($tmp_check_content) !== '') {
    ...
    if ($intro_dispDefault && !isset($_GET['scenario']) && api_is_allowed_to_edit()) {
        //$intro_content = make_clickable($intro_content); // make url in text clickable
        $intro_content = text_filter($intro_content); // parse [tex] codes
        if (!empty($intro_content) )	{
            echo '<div id="courseintroduction"><div class="scroll_feedback">';    
...
if ($intro_dispCommand  && !isset($_GET['scenario'])) {
	...
	if ( empty($intro_content) ) {
		...
		if (!empty ($GLOBALS["_cid"])) {
			echo "<a href=\"".api_get_self()."?".api_get_cidreq()."&amp;intro_cmdDel=1&amp;course_scenario=1\" onclick=\"javascript:if(!confirm('".addslashes(api_htmlentities(get_lang('ConfirmYourChoice'),ENT_QUOTES,$charset))."')) return false;\">".Display::return_icon('pixel.gif', get_lang('Delete'), array('class' => 'actionplaceholdericon actiondelete', 'style'=>'vertical-align:middle')).' '.get_lang('Delete').' '.get_lang('Scenario').'</a>' . PHP_EOL;
			echo "<a href=\"".api_get_self()."?".api_get_cidreq()."&amp;intro_cmdEdit=1&amp;course_scenario=1".$add_param_display_template."\">".Display::return_icon('pixel.gif', get_lang('Modify'), array('class' => 'actionplaceholdericon actionedit', 'style'=>'vertical-align:middle')).' '.get_lang('Edit').' '.get_lang('Scenario').'</a>' . PHP_EOL;
		} else {
			echo "<a href=\"".api_get_self()."?intro_cmdDel=1&amp;course_scenario=1\" onclick=\"javascript:if(!confirm('".addslashes(api_htmlentities(get_lang('ConfirmYourChoice'),ENT_QUOTES,$charset))."')) return false;\">" . Display::return_icon('pixel.gif', get_lang('Delete'), array('class' => 'actionplaceholdericon actiondelete', 'style'=>'vertical-align:middle')).' '.get_lang('Delete').' '.get_lang('Scenario'). '</a>' . PHP_EOL;
			echo "<a href=\"".api_get_self()."?intro_cmdEdit=1&amp;course_scenario=1\">".Display::return_icon('pixel.gif', get_lang('Edit'), array('class' => 'actionplaceholdericon actionedit', 'style'=>'vertical-align:middle')).' '.get_lang('Edit').' '.get_lang('Scenario').'</a>' . PHP_EOL;
		}
		echo "</div>";
