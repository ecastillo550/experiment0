...
<?php 
		...
		if(isset($_SESSION['isShoppingCartActive']) && $_SESSION['isShoppingCartActive'] == TRUE ) :
...
?>
			            <div id="top_corner"><a style="display:block;height:50px;width:200px;"  href="<?php echo api_get_path(WEB_PATH);?>index.php"></a></div>
						<div id="institution">
	...
	<?php if (!empty($_course) && empty($_GET['student'])){ ?>
	<div id="header3">
		<div class="headerinner">
...
if ($_user['user_id'] && !api_is_anonymous()) {
    ...
    // Search
    if (api_get_setting('show_tabs', 'search') == 'true') {
        $navigation['search'] = $possible_tabs['search'];
    } else {
        $menu_navigation['search'] = $possible_tabs['search'];
    }
}

// the tabs menu DOKEOS    
echo '
<div id="dokeostabs">                                                                                      				                               
      ...
      if (api_get_setting('search_enabled') == 'true' && extension_loaded('xapian') && !api_is_anonymous()) {
       ?>    
                  <form  method="POST" onsubmit="return h_search();" style="position:relative;">
                       <span>
                           <input  type="text" id="input" name="input" class="input-h-search" id="search-text-input">                               
                       </span>
                       <span>
                           <input style="" type="image" src="<?php echo api_get_path(WEB_IMG_PATH).'button_search.png'; ?>" id="btn-h-search"/>
                       </span>
                    </form>
        <?php
         }
...
if ($_user['user_id']) {
    ...
    echo '	<li class="logout"><a class="logoutClick"  href="" >'.  get_lang('Logout') .' '.$login.'</a></li>';
}    
...
echo '<center><img src="'. api_get_path(WEB_IMG_PATH).'logout-tab.png" style="vertical-align:text-bottom;"><br/>'.get_lang('AreYouSureAreCloseSession').'</center>'; 
echo '</div>';
