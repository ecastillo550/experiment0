		...
		if (Database::num_rows($result) > 0) {
			...
			if (!empty($_SESSION['request_uri'])) {
				...
				if (isset($param)) {
					...
					if (api_get_setting('force_password_change')<> 0 OR $uData['login_counter'] == '-1'){
						force_password_change($uData);						
						update_login_counter($uData['user_id']);
					} else {
...
if ((!empty($_SESSION['force_password_change']) && $_SESSION['force_password_change'] == true AND !empty($_user)) OR (isset($_user['login_counter']) && $_user['login_counter'] == '-1')){
	force_password_change($_user);
}
