...
require_once $lib_path.'help.lib.php';
...
// initialisation of the variable that holds all the delegated settings
$_settingdelegation = array();
...
foreach ($result as $row) {
	...
	// we also want to know which settings can be delegated to the course
	if ($row['scope'] == '1'){
		// the procedure for adding a setting to the $_settingdelegation array depends on the existence of subkeys
		// if subkeys do not exist (radio button, text input, ... ) then we can simply add them
		// if subkeys DO exist (checkboxes) then we first check if the variable already exists in the $_settingdelegation array. 
		// If it does already exist we only add the subkey and the subkeytext to its respective array elements
		// if it does not already exist we add it completely (so that for the next iteration we arrive in the previous scenario
		if ($row['subkey'] == null) {
			$_settingdelegation[$row['variable']]=$row;
		} else {
			if (key_exists($row['variable'],$_settingdelegation)){
				$_settingdelegation[$row['variable']]['subkey'][] = $row['subkey'];
				$_settingdelegation[$row['variable']]['subkeytext'][] = $row['subkeytext'];
			} else {
				$_settingdelegation[$row['variable']]=$row;
				$_settingdelegation[$row['variable']]['subkey'] = array($row['subkey']);
				$_settingdelegation[$row['variable']]['subkeytext'] = array($row['subkeytext']);
			}
		}
	}
}

// getting the plugins from the settings table
...
/*
 * When acess control is activated in ie, every page loaded check the homepage (index.php).
 * That means the informations of the course in the session are lost. 
 * We have to block it checking on index.php if referer is null and time between the two request ridiculous
 */
if(defined('DOKEOS_HOMEPAGE') && DOKEOS_HOMEPAGE === true && api_get_setting('access_control_activated') == 'true'){
	if(empty($_SERVER['HTTP_REFERER']) &&  time() - $_SESSION['last_page_loaded_time'] < 5){
		die();
	}
}
...
if (!empty($_course) AND is_array($_course)){
	api_register_delegated_settings();
}
...
// load libraries for mvc model
require_once $lib_path.'app_controller.php';
require_once $lib_path.'app_view.php';
...
if (api_get_setting('server_type') == 'test') {
	...
	*/
	...
	// TODO: These obsolete variables $HTTP_* have to be checked whether they are actually used.
...
// Allow load the homepage in the selected language from admin portal
if (isset($_user['language']) && api_get_self() != api_get_path(REL_PATH).'main/admin/configure_homepage.php') {
 ...
 * We have to block it checking on index.php if referer is null and time between the two request ridiculous
 */
if(api_get_setting('access_control_activated') == 'true'){
	$_SESSION['last_page_loaded_time'] = time();
}
