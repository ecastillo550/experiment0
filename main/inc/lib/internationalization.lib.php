...
define('DATE_FORMAT_SHORT_WITH_LONG_YEAR', 4);		// Aug 25, 2009
//
...
function api_get_interface_language($purified = false) {
	global $language_interface;
	$current_language = empty($language_interface) ? 'english' : ($purified ? api_purify_language_id($language_interface) : $language_interface);
        
        // Check if current language has sublanguage
        /*$language_id = api_get_language_id($current_language, false);
        ...
        if (!empty($sublanguages_info['dokeos_folder'])) {
            $current_language = $sublanguages_info['dokeos_folder'];
        }*/
	...
	if (is_int($date_format)) {
		switch ($date_format) {
			...
			case DATE_TIME_FORMAT_LONG:
				...
				break;
			case DATE_FORMAT_SHORT_WITH_LONG_YEAR:
				$date_format = get_lang('dateFormatShortWithLongYear', '', $language);
				break;
			default:
				$date_format = get_lang('dateTimeFormatLong', '', $language);
		}
