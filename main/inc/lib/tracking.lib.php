	...
	function get_last_connection_date_on_the_course($student_id, $course_code) {
		...
		if(Database::num_rows($rs)>0)
		{
			if ($last_login_date = Database::result($rs, 0, 0)) {
				...
				//345600 = 7 days in seconds
				if ($currentTimestamp - $timestamp > 604800) {
					return format_locale_date(get_lang('dateFormatShortWithLongYear'), strtotime($last_login_date)) . (api_is_allowed_to_edit()?' <a href="'.api_get_path(WEB_VIEW_PATH).'announcement/index.php?action=add&remind_inactive='.$student_id.'" title="'.get_lang('RemindInactiveUser').'"><img align="middle" src="'.api_get_path(WEB_IMG_PATH).'messagebox_warning.gif" /></a>':'');
				} else {
