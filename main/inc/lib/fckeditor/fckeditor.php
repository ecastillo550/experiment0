...
class FCKeditor
{
	...
	private function & get_css_configuration() {
                ...
                if (file_exists(api_get_path(SYS_PATH) . 'main/css/' . api_get_setting('stylesheets') . '/templates.css')) {
                    $config['EditorAreaCSS'] = api_get_path(REL_PATH).'main/css/'.api_get_setting('stylesheets').'/templates.css';        
                }                
	...
	private function & get_repository_configuration() {
		...
		if (api_is_in_course()) {
			...
			if (api_is_platform_admin() && $_SESSION['this_section'] == 'platform_admin') {
                ...
                // Just for 2.0 beta, we need to create a shared repository for the images,icons(media resources)
				$config['CreateDocumentWebDir'] = api_get_path(WEB_PATH).'main/default_course_document/';
				$config['CreateDocumentDir'] = api_get_path(WEB_PATH).'main/default_course_document/'; // A side-effect is in use here.
				$config['BaseHref'] = api_get_path(WEB_PATH).'main/default_course_document/';
