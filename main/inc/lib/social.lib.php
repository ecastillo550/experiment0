...
class SocialManager extends UserManager {
		...
		if ($_GET['id'] == '') {
			...
			foreach ($user_list as $user) {
				...
				if ($user_info['user_id'] != api_get_user_id()) {
				  $table_row[] = '<div style="cursor:pointer;" id="chat_'.$user_info['username'].'" class="chat_friend">'.api_get_person_name(cut($user_info['firstName'],15), cut($user_info['lastName'],15)).'<br/>'.Display::return_icon('pixel.gif', get_lang('Chat'),array('class'=>'toolchatsocialtalk')).'</div>';
				} else {
				  $table_row[] = '<div id="chat_'.$user_info['username'].'" >'.api_get_person_name(cut($user_info['firstName'],15), cut($user_info['lastName'],15)).'</div>';
				}
    ...
    public static function display_chat_useronline($user_list) {
        ...
        // avoid in some tools
        if (defined('DOKEOS_EXERCISE') && DOKEOS_EXERCISE) {
            return false;
        }
       ...
       //echo '<script type="text/javascript" src="'.api_get_path(WEB_LIBRARY_PATH).'javascript/jquery-1.4.2.min.js" language="javascript"></script>';
        ...
        echo '<script type="text/javascript">
            $(document).ready(function() {
                ...
                $(".usersonlineicon").css("cursor","pointer").click(function(){
                    $("#chat-dialog").dialog({modal: true, title: "'.get_lang('UsersOnline').'",width: "600px"});
                });
             });
        </script>';
        // chat
