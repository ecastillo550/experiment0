 ...
 */
function api_display_language_form($hide_if_no_choice = false, $style = 'none', $add_params = false) {
	$platformLanguage = api_get_setting('platformLanguage');
...
<?php
	...
	foreach ($original_languages as $key => $value) {
		...
		echo "<option value=\"$params\"$option_end";
...
function force_password_change($user){
	...
	if (api_get_setting('force_password_change') == 0) {
		return true;
	}
