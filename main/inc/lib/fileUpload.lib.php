...
function handle_uploaded_document($_course,$uploaded_file,$base_work_dir,$upload_path,$user_id,$to_group_id=0,$to_user_id=NULL,$maxFilledSpace='',$unzip=0,$what_if_file_exists='',$output=true)
{
	...
	{
		echo get_lang('UplNotEnoughSpace');
		return false;
	}
...
/**
 * this function does the save-work for the documents.
 * it handles the multiple uploaded file and adds the properties to the database
 * if unzip=1 and the file is a zipfile, it is extracted
 * if we decide to save ALL kinds of documents in one database,
 * we could extend this with a $type='document', 'scormdocument',...
 *
 ...
 * @return path of the saved file
 */
function handle_multiple_uploaded_document($_course,$uploaded_file,$file_no,$base_work_dir,$upload_path,$user_id,$to_group_id=0,$to_user_id=NULL,$maxFilledSpace='',$unzip=0,$what_if_file_exists='',$output=true)
{	
	if(!$user_id) die("Not a valid user.");

	//strip slashes
	$uploaded_file['name'][$file_no]=stripslashes($uploaded_file['name'][$file_no]);
	//add extension to files without one (if possible)
	$uploaded_file['name'][$file_no]=add_ext_on_mime($uploaded_file['name'][$file_no],$uploaded_file['type'][$file_no]);
	$current_session_id = api_get_session_id();
	//check if there is enough space to save the file
	if (!enough_space($uploaded_file['size'][$file_no], $maxFilledSpace)) {
		//echo get_lang('UplNotEnoughSpace');
		return false;
	}

	//if the want to unzip, check if the file has a .zip (or ZIP,Zip,ZiP,...) extension
	if ($unzip == 1 && preg_match("/.zip$/", strtolower($uploaded_file['name'][$file_no])) ) {
		return unzip_uploaded_document($uploaded_file, $upload_path, $base_work_dir, $maxFilledSpace, $output, $to_group_id,$file_no);
		//display_message("Unzipping file");
	}
	//we can only unzip ZIP files (no gz, tar,...)
	elseif ($unzip == 1 && !preg_match("/.zip$/", strtolower($uploaded_file['name'][$file_no])) ) {
		//Display::display_error_message(get_lang('UplNotAZip')." ".get_lang('PleaseTryAgain'));
		//echo get_lang('UplNotAZip')." ".get_lang('PleaseTryAgain');
		return false;
	} else {
		//clean up the name, only ASCII characters should stay. (and strict)
		$clean_name = replace_dangerous_char($uploaded_file['name'][$file_no], 'strict');
		//no "dangerous" files
		$clean_name = disable_dangerous_file($clean_name);
		if(!filter_extension($clean_name)) {
			return false;
			...
			$store_path = $where_to_save.$clean_name;
			//name of the document without the extension (for the title)
		//	$document_name = get_document_title($uploaded_file['name'][$file_no]);
			$document_name = $uploaded_file['name'][$file_no];
			//size of the uploaded file (in bytes)
			$file_size = $uploaded_file['size'][$file_no];
			...
			switch ($what_if_file_exists) {
				...
				case 'overwrite':                                        
					...
					if (@move_uploaded_file($uploaded_file['tmp_name'][$file_no], $store_path)) {                                            
						chmod($store_path,$files_perm);
						if($file_exists) {
							//UPDATE DATABASE!
							$document_id = DocumentManager::get_document_id($_course,$file_path);
							if ($document_id) {
								//update filesize
								update_existing_document($_course,$document_id,$uploaded_file['size'][$file_no]);
								//update document item_property
								api_item_property_update($_course,TOOL_DOCUMENT,$document_id,'DocumentUpdated',$user_id,$to_group_id,$to_user_id,null,null,$current_session_id);
							}
							//if the file is in a folder, we need to update all parent folders
							...
							if($output){
								//Display::display_confirmation_message(get_lang('UplUploadSucceeded')."<br/>".$file_path .' '. get_lang('UplFileOverwritten'),false);
								//echo get_lang('UplUploadSucceeded')."<br/>".$file_path .' '. get_lang('UplFileOverwritten');
							}
							return $file_path;
						} else {
							//put the document data in the database
...
//						Display::display_error_message(get_lang('UplUnableToSaveFile'));
//						echo get_lang('UplUnableToSaveFile');
						return false;
					}
					break;
				...
				case 'rename':
					...
					if (@move_uploaded_file($uploaded_file['tmp_name'][$file_no], $store_path))
					{
						...
						if($output){
							//Display::display_confirmation_message(get_lang('UplUploadSucceeded'). "<br>" .get_lang('UplFileSavedAs') . $new_file_path,false);
							//echo get_lang('UplUploadSucceeded'). "<br>" .get_lang('UplFileSavedAs') . $new_file_path;
						}
						return $new_file_path;
					}
					else {
...
function unzip_uploaded_document($uploaded_file, $upload_path, $base_work_dir, $max_filled_space, $output = true, $to_group_id=0, $file_no = -1) {
	global $_course;
	global $_user;
	global $to_user_id;
	global $to_group_id;
    if ($file_no >= 0) {
      $zip_file = $uploaded_file['tmp_name'][$file_no];
    } else {
      $zip_file = $uploaded_file['tmp_name'];
    }
	$zip_file = new pclZip($zip_file);
	...
	*/
	...
	//echo get_lang('UplZipExtractSuccess');
	return true;
	/*
	if ($upload_path != '/')
		$upload_path = $upload_path.'/';
