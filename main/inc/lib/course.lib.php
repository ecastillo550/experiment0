...
class CourseManager {
	...
	*/
	...
	public static function delete_course($code) {
		...
		EcommerceCatalog::create()->deleteCourseEcommerce($code);		
	...
	public static function get_details_course_description_html($descriptions, $charset, $action_show = true) {
		if (isset($descriptions) && count($descriptions) > 0) {
			$data = '<div class="course_description bg_white" >';
			foreach ($descriptions as $id => $description) {
				$data .= '<div class="section_white">';
				if (api_is_allowed_to_edit() && $action_show == true) {
					...
					$data .= Display::return_icon('delete.png', get_lang('Delete'), array('style' => 'vertical-align:middle;float:right;'));
					$data .= '</a> ';
					//edit
					$data .= '<a href="'.api_get_self().'?'.api_get_cidreq().'&amp;description_id='.$description->id.'">';
					$data .= Display::return_icon('edit.png', get_lang('Edit'), array('style' => 'vertical-align:middle;float:right; padding-right:4px;'));
					$data .= '</a> ';
				}
				$data .= '<div class="sectiontitle">'.$description->title.'</div>';
				$data .= '<div class="sectionContent" valign="top">';
			...
			$data .= '<div class="course_description bg_white"><div class="section_white">'.get_lang('ThisCourseDescriptionIsEmpty').'</div></div>';
		}
	...
	*/
	...
	/**
	 * Get the number of courses inside a category.
	 *
    ...
    function get_enrolment_and_no_enrolment_course_list ($enrolment_type = 0) {
     $course_table = Database::get_main_table(TABLE_MAIN_COURSE);
     $url_table = Database::get_main_table(TABLE_MAIN_ACCESS_URL_REL_COURSE);

      global $_configuration;
      if ($_configuration['multiple_access_urls']==true) {
       $url_id = api_get_current_access_url_id();
       $sql = "SELECT code,title,visual_code FROM $course_table c INNER JOIN $url_table u ON c.code=u.course_code
         WHERE u.access_url_id= '$url_id' AND c.default_enrolment = '$enrolment_type' ORDER BY c.title ASC";
      } else {
       $sql = "SELECT code,title,visual_code FROM $course_table c WHERE c.default_enrolment = '$enrolment_type' ORDER BY c.title ASC";
      }
     $rs = Database::query($sql, __FILE__, __LINE__);
     $course_list = array();
     while ($row = Database::fetch_array($rs, 'ASSOC')) {
       $course_list[$row['code']] = $row;
     }
     return $course_list;
    }
    /**
