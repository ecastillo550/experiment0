...
*	@author Roan Embrechts (Vrije Universiteit Brussel), some cleaning
...
// The tools of a group can have 3 states: not available, public, private
...
// - DEFAULT_GROUP_CATEGORY: When group categories aren't available (platformsetting),  all groups are created in this 'dummy'-category
...
class GroupManager {
	...
	public static function get_group_properties ($group_id) {
		...
		if (is_numeric($group_id)){
			$sql = 'SELECT i.id,i.name,i.category_id,i.description,i.max_student,i.doc_state,i.calendar_state,i.work_state,i.announcements_state,i.forum_state,i.wiki_state,i.chat_state, i.secret_directory,i.self_registration_allowed,i.self_unregistration_allowed,i.session_id,c.group_state  FROM '.$table_group.' i LEFT JOIN '.$table_group_cat.' c ON i.category_id=c.id WHERE i.id = '.Database::escape_string($group_id);
		} elseif($group_id=='all'){
		...
		while ($db_object = Database::fetch_object($db_result)){
			$result['id'] = $db_object->id;
			$result['name'] = $db_object->name;
			$result['category_id'] = $db_object->category_id;
			...
			$result['doc_state'] = $db_object->doc_state;
                        $result['group_state'] = $db_object->group_state;
	...
	public static function set_group_properties ($group_id, $name, $description, $maximum_number_of_students, $doc_state, $work_state, $calendar_state, $announcements_state, $forum_state, $wiki_state, $chat_state, $self_registration_allowed, $self_unregistration_allowed) {
		...
		$sql = "UPDATE ".$table_group."
					...
					WHERE id=".Database::escape_string($group_id);
		...
		$sql2 .=" WHERE forum_of_group=".Database::escape_string($group_id);
	...
	*	GROUPCATEGORY FUNCTIONS
	...
	public static function get_category ($id, $course_code = null) {
		...
		$table_group_cat = Database :: get_course_table(TABLE_GROUP_CATEGORY, $course_db);
		$sql = "SELECT * FROM $table_group_cat WHERE id = ".Database::escape_string($id);
		$res = Database::query($sql,__FILE__,__LINE__);
	 ...
	 */
	public static function delete_category ($cat_id, $course_code = null, $deletegroups = false) {
		$course_db = '';
		if ($course_code != null)
		{
			$course_info = Database :: get_course_info($course_code);
		...
		// Delete the containing groups or simply move them outside the category
		if ($deletegroups == true) {
			$sql = "SELECT id FROM $table_group WHERE category_id='".Database::escape_string($cat_id)."'";
			$res = Database::query($sql,__FILE__,__LINE__);
			...
			$sql = "UPDATE $table_group SET category_id = 0 WHERE category_id='".Database::escape_string($cat_id)."'";
			$res = Database::query($sql,__FILE__,__LINE__);
		}

		// Delete the group category
		$sql = "DELETE FROM $table_group_cat WHERE id='".Database::escape_string($cat_id)."'";
		Database::query($sql,__FILE__,__LINE__);
	}
	/**
	 ...
	 */
	public static function create_category ($title, $description, $doc_state, $work_state, $calendar_state, $announcements_state, $forum_state, $wiki_state, $chat_state, $self_registration_allowed, $self_unregistration_allowed, $maximum_number_of_students, $groups_per_user, $group_state = 0) {
		...
		$sql = "INSERT INTO ".$table_group_category."
					...
					group_state = '".Database::escape_string($group_state)."',
					max_student = '".Database::escape_string($maximum_number_of_students)."' ";
		Database::query($sql,__FILE__,__LINE__);
	...
	*	GROUP USERS FUNCTIONS
	...
	/**
	 * Fill the groups with students. - This is function is changed to old and added new one below
	 ...
	 * @return void
	 */
	public static function fill_groups_old ($group_ids) {
		$group_ids = is_array($group_ids) ? $group_ids : array ($group_ids);
		...
		// get the number of groups a user can be subscribed to
	        $number_groups_per_user = ($groups_per_user == GROUP_PER_MEMBER_NO_LIMIT ? INFINITE : $groups_per_user);

		// get all the groups where enrollment is still allowed, which means all the groups that have to reached the maximum number of members (ordered by the number of places available)
		...
		// Retrieve course users (reverse) ordered by the number of group they are already enrolled 
		// @todo: this is not performant because for every user in the course a query is executed
		...
		// getting the number of groups the users can still subscribe to (number of groups per users as defined by the category minus the groups already subscribed to)
		$sql = "SELECT count(tblgroupuser.group_id) AS number_group_memberships, tblgroupuser.user_id, tblgroup.category_id FROM $group_user_table tblgroupuser, $group_table tblgroup 
				WHERE tblgroupuser.group_id = tblgroup.id AND tblgroup.category_id = '".Database::escape_string($category['id'])."' GROUP BY tblgroupuser.user_id";
		$result = Database::query($sql,__FILE__,__LINE__);
		while ($row = Database::fetch_array($sql_result, 'ASSOC'))
		{
			$complete_user_list[$row['user_id']]['number_groups_left'] = $number_groups_per_user - $row['number_group_memberships'];
		}
	...
	/*New funciton added to fill groups - this function doesnot copy one user in different groups.
	one user can be in one group */
	public static function fill_groups ($group_id) {
		global $_course;
		// Database table definitions
		$course_user_table 	= Database :: get_main_table(TABLE_MAIN_COURSE_USER);
		$group_table 		= Database :: get_course_table(TABLE_GROUP);
		...
		foreach ($complete_user_list as $index => $user) {
			$complete_user_id[] = $user['user_id'];
		}
		                
		$group_user_id = array();
		$sql = "SELECT DISTINCT(gu.user_id) AS user_id FROM $group_table gp,$group_user_table gu WHERE gp.id = gu.group_id AND gu.group_id = ".intval($group_id);
		...
		$current_available_users = array();
		$current_available_users = array_diff($complete_user_id,$group_user_id);
		$current_available_users = array_merge($current_available_users);				

		$sql = "SELECT max_student FROM $group_table WHERE id=".$group_id;
		$rs = Database::query($sql,__FILE__,__LINE__);
		while($row = Database::fetch_row($rs)){
			$max_student = $row[0];			
		}

		$sql = "SELECT count(gu.user_id) as num_users FROM $group_table gp,$group_user_table gu where gp.id=gu.group_id and gu.group_id=".$group_id;
		$rs = Database::query($sql,__FILE__,__LINE__);
		while($row = Database::fetch_row($rs)){			
			$num_users_present = $row[0];
		}
		
		foreach($current_available_users as $user_id){			
			if($num_users_present < $max_student){
				if (self :: can_user_subscribe($user_id, $group_id))
				{					
					self :: subscribe_users($user_id, $group_id);
					$num_users_present++;					
	...
	public static function user_in_number_of_groups ($user_id, $cat_id) {
		...
		$sql = 'SELECT  COUNT(*) AS number_of_groups FROM '.$table_group_user.' gu, '.$table_group.' g WHERE gu.user_id = \''.Database::escape_string($user_id).'\' AND g.id = gu.group_id AND g.category_id=  \''.Database::escape_string($cat_id).'\'';
		$db_result = Database::query($sql,__FILE__,__LINE__);
		$db_object = Database::fetch_object($db_result);
	...
	public static function is_self_registration_allowed ($user_id, $group_id) {
		...
		if (isset($group_id)) {
			$sql = 'SELECT  self_registration_allowed FROM '.$table_group.' WHERE id = "'.Database::escape_string($group_id).'" ';
			$db_result = Database::query($sql,__FILE__,__LINE__);
			$db_object = Database::fetch_object($db_result);
	...
	public static function can_user_subscribe ($user_id, $group_id) {
		...
		$result = CourseManager :: is_user_subscribed_in_real_or_linked_course($user_id, $_course['sysCode']);

		// check if the user is subscribed to the course
		$result = !self :: is_subscribed($user_id, $group_id);

		// check if the limit of number of users of the group is not exceeded
	...
	public static function get_group_users ($group_id) {
		...
		$sql = "SELECT `ug`.`id`, `u`.`user_id`, `u`.`lastname`, `u`.`firstname`, `u`.`email`, `u`.`username`
			FROM ".$table_user." u, ".$table_group_user." ug
			WHERE `ug`.`group_id`='".$group_id."'
			AND `ug`.`user_id`=`u`.`user_id` AND `ug`.`user_id` <> ".api_get_user_id(). $order_clause;
		$db_result = Database::query($sql,__FILE__,__LINE__);
		$users = array ();
		while ($user = Database::fetch_object($db_result))
		{			
			$users[$user->user_id] = $user->firstname.' '.$user->lastname.' ('.$user->username.')';
		}
		return $users;
	}

	/**
	 * Get all tutors from a group
	 * @param int $group_id
	 * @return array An array with information of all users from the given group.
	 *               (user_id, firstname, lastname, email)
	 */
	public static function get_group_tutors ($group_id) {
		$table_user = Database :: get_main_table(TABLE_MAIN_USER);
	 ...
	 *               (user_id, firstname, lastname, email)
	 */
	public static function get_group_tutorslist ($group_id) {
		$table_user = Database :: get_main_table(TABLE_MAIN_USER);
		$table_group_tutor = Database :: get_course_table(TABLE_GROUP_TUTOR);
		$order_clause = api_sort_by_first_name() ? ' ORDER BY `u`.`firstname`, `u`.`lastname`' : ' ORDER BY `u`.`lastname`, `u`.`firstname`';
		$group_id = Database::escape_string($group_id);
	 ...
	 *               (user_id, firstname, lastname, email)
	 */
	public static function get_othergroup_tutors ($group_id) {
		$table_user = Database :: get_main_table(TABLE_MAIN_USER);
		$table_group_tutor = Database :: get_course_table(TABLE_GROUP_TUTOR);
		$order_clause = api_sort_by_first_name() ? ' ORDER BY `u`.`firstname`, `u`.`lastname`' : ' ORDER BY `u`.`lastname`, `u`.`firstname`';
		$group_id = Database::escape_string($group_id);
	...
	public static function subscribe_tutors ($user_ids, $group_id) {
		...
		foreach ($user_ids as $index => $user_id)
		{
			...
			Database::query("INSERT INTO ".$table_group_tutor." (user_id, group_id) VALUES ('".$user_id."', '".$group_id."')");
		}
		return $result;
	}
	...
	public static function unsubscribe_users ($user_ids, $group_id) {
            $user_ids = is_array($user_ids) ? $user_ids : array ($user_ids);
            $table_group_user = Database :: get_course_table(TABLE_GROUP_USER);
            Database::query('DELETE FROM '.$table_group_user.' WHERE group_id = '.intval($group_id).' AND user_id IN ('.implode(',', $user_ids).')');
	}
	...
	*/
	...
	public static function is_tutor ($user_id) {
		...
		$sql = "SELECT tutor_id FROM ".$course_user_table."
		             ...
		             AND `course_code`='".Database::escape_string($_course['sysCode'])."'"."AND tutor_id=1";
		$db_result = Database::query($sql,__FILE__,__LINE__);
	...
	public static function get_group_ids ($course_db,$user_id) {
		...
		$sql = "SELECT group_id FROM $tbl_group WHERE user_id = '".Database::escape_string($user_id)."'";
	...
	public static function get_selected_group_info_of_subscribed_groups($course_db,$user_id,$fields=array()){
		// variable initialisation
		$groups = array();
		...
		$sql = 'SELECT  '.$fieldssql.' 
			...
			WHERE gu.user_id = \''.Database::escape_string($user_id).'\'
	...
	*/
	...
	/**
	 * Get the group name of all the groups the user is subscribed to
	 */
	public static function get_user_group_name ($user_id) {
		...
		$sql_groups = 'SELECT name FROM '.$table_group.' g,'.$table_group_user.' gu WHERE gu.user_id="'.Database::escape_string($user_id).'" AND gu.group_id=g.id';
	...
	/**
	 * This function has double functionality. 
	 * Either you can get all the groups and the users registered in that group ($key = 'group')
	 * Or you can get all the users and the groups the user is registered to ($key = 'user')
	 */

	public static function get_all_groups_and_users($key='group',$userinfo = array()){
		// database table definition
		$table_group		= Database :: get_course_table(TABLE_GROUP);
		$group_user_table	= Database :: get_course_table(TABLE_GROUP_USER);
		$table_user			= Database :: get_main_table(TABLE_MAIN_USER);

		$sql = "SELECT user_id, group_id FROM ".$group_user_table;
		if (!empty($userinfo)){
			$sql = "SELECT groupreluser.user_id, group_id, ".implode(',',$userinfo)." FROM ".$group_user_table." groupreluser LEFT JOIN $table_user user on groupreluser.user_id = user.user_id";
		}
		$result = Database::query($sql,__FILE__,__LINE__);
		while ($row = Database::fetch_array($result, 'ASSOC'))
		{
			if ($key == 'group'){
				if (empty($userinfo)){
					$return[$row['group_id']][] = $row['user_id'];
				} else {
	...
	function get_mygroups($user_id) {
		// Database table definition
		$table_group_user 	= Database :: get_course_table(TABLE_GROUP_USER);
		$table_group 		= Database :: get_course_table(TABLE_GROUP);
		$table_group_category 	= Database :: get_course_table(TABLE_GROUP_CATEGORY);

		$sql = 'SELECT g.id , g.name , c.title, gu.role '
				.' FROM '. $table_group  .' g , '. $table_group_category  .' c , '. $table_group_user  .' gu '
				. 'WHERE g . id = gu . group_id AND g . category_id = c . id AND gu . user_id = \'' . Database::escape_string($user_id). '\'';
	...
	//Function to update group category table for adding tutoring scenario in group category if not exists.
	function update_group_category(){
		$table_group_category 	= Database :: get_course_table(TABLE_GROUP_CATEGORY);

		$sql = "SELECT * FROM $table_group_category";
		$res = Database::query($sql,__FILE__,__LINE__);
		$num_rows = Database::num_rows($res);
		if($num_rows == 2){
			$sql_ins = "INSERT INTO $table_group_category(title,description,icon,self_reg_allowed,self_unreg_allowed) VALUES('Tutoring','In a tutoring scenario student can interact with tutor and not with group members','group.png',1,1)";
			Database::query($sql_ins,__FILE__,__LINE__);
			$sql_upd = "UPDATE $table_group_category SET id = 4 WHERE title = 'Competition'";
			Database::query($sql_upd,__FILE__,__LINE__);
			$sql_upd = "UPDATE $table_group_category SET id = 1 WHERE title = 'Tutoring'";
			Database::query($sql_upd,__FILE__,__LINE__);
			$sql_upd = "UPDATE $table_group_category SET id = 3 WHERE title = 'Collaboration'";
			Database::query($sql_upd,__FILE__,__LINE__);
			$sql_upd = "UPDATE $table_group_category SET id = 2 WHERE title = 'Competition'";
			Database::query($sql_upd,__FILE__,__LINE__);
		}
	}
}
