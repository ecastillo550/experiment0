...
class SessionManager {
    ...
    public static function fetch($id) {
	  ...
	  * @return $id_session;
	  **/
	public static function create_session ($sname,$sdescription,$syear_start,$smonth_start,$sday_start,$syear_end,$smonth_end,$sday_end,$nolimit,$coach_username, $id_session_category,$cost, $certif_template = 0, $certificate_tool = '', $certif_min_score = 0.00, $certif_min_progress = 0.00) {
		global $_user;
		$name= trim($sname);
	...
	**/
	public static function edit_session ($id,$name,$description,$year_start,$month_start,$day_start,$year_end,$month_end,$day_end,$nolimit,$id_coach, $id_session_category,$cost, $certif_template = 0, $certificate_tool = '', $certif_min_score = 0.00, $certif_min_progress = 0.00) {
		global $_user;
		...
		foreach ($course_list as $enreg_course) {
			...
			foreach ($user_list as $enreg_user) {
				if(!in_array($enreg_user, $existingUsers)) {					
						...
						/*	foreach ($check_priority_list as $each_priority_info) {
								if ($each_priority_info[1] == $enreg_course && $each_priority_info[0] == $id_session) {
									$insert_sql = "INSERT IGNORE INTO $tbl_session_rel_course_rel_user(id_session,course_code,id_user,visibility) VALUES('$id_session','$enreg_course','$enreg_user','$session_visivility')";
									Database::query($insert_sql,__FILE__,__LINE__);
									if(Database::affected_rows()) {
											$nbr_users++;
 ...
 */
	...
	* */
	...
	/**
	  ...
	  * @return $id_session;
	  **/
	public static function create_category_session ($sname,$sdescription,$syear_start,$smonth_start,$sday_start,$syear_end,$smonth_end, $sday_end, $cost = 0, $method_payment = '', $code = '') {
            ...
            if (empty($name)) {
                ...
                $sql = "INSERT INTO $tbl_session_category(name, description, date_start, date_end, cost, method_payment, code) VALUES('".Database::escape_string($name)."','".Database::escape_string($description)."','$date_start','$date_end', '$cost', '".Database::escape_string($method_payment)."', '".Database::escape_string($code)."')";
                Database::query($sql ,__FILE__,__LINE__);
                $id_session=Database::insert_id();
                // add event to system log
                $time = time();
	 ...
	 * The parameter id is a primary key
	**/
	public static function edit_category_session($id, $sname, $sdescription, $syear_start,$smonth_start,$sday_start,$syear_end,$smonth_end, $sday_end, $cost = 0, $method_payment = '', $code = ''){
		$tbl_session_category = Database::get_main_table(TABLE_MAIN_SESSION_CATEGORY);
		$name= trim($sname);
		$description= trim($sdescription);
		$year_start= intval($syear_start);
		...
		if (empty($name)) {
			...
			$sql = "UPDATE $tbl_session_category SET 
                                 ...
                                 date_end = '$date_end', 
                                 cost = '$cost', 
                                 method_payment = '".Database::escape_string($method_payment)."',
	...
	*/
	...
	public static function is_session_coach($user_id, $session_id){
         ...
         * @param   int     user id 
         */
         public static function automatically_add_user_to_group($course_code, $user_id) {
            ...
            if (!$affected_rows) {
                if (api_get_setting('create_new_group') == 'true') {
                    ...
                    if (Database::num_rows($check) == 0) {
                        ...
                        //Email
                        $sql = "SELECT content FROM $emailtemplate_table WHERE description = 'NewGroup' AND language='".api_get_interface_language()."'";
                        $res = Database::query($sql,__FILE__,__LINE__);
                        $num_rows = Database::num_rows($res);
                        if($num_rows == 0){
                        $sql = "SELECT content FROM $emailtemplate_table WHERE description = 'NewGroup' AND language = 'english'";
                        $res = Database::query($sql,__FILE__,__LINE__);
                        }
        ...
        public static function send_email_about_new_session_to_administrators($session_id, $user_id, $cat_id) {
            ...
            $sql = "SELECT * FROM $table_emailtemplate WHERE description = 'UserRegistrationToSession' AND language= '".$language_interface."'";
        ...
        public static function send_email_to_registered_user_with_cc_or_intallment($user_params, $cat_id) {
            ...
            $content =  str_replace('{administratorSurname}', api_get_person_name(api_get_setting('administratorName'), api_get_setting('administratorSurname')), $content);
            $content =  str_replace('{detailsUrl}', '<a href="http://formation-publique.fr" target="_blank">http://formation-publique.fr</a>', $content);
