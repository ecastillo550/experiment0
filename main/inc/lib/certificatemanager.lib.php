...
class CertificateManager {
      ...
      public function redirectPage($action, $lang = '') {
          ...
          echo '<script>location.href="'.api_get_path(WEB_CODE_PATH).'admin/certificate_templates.php?action='.$action.(!empty($lang)?'&lang='.$lang:'').'"</script>';
          return false;
      }
       ...
       * @return    void       
       */
      public function returnCertificateThumbnailImg($tpl_id) {
          $image = '';
          if (!empty($tpl_id)) {
              $certificate = $this->getCertificateInfo($tpl_id);
              ...
              if (!empty($certificate['thumbnail'])) {                      
                  ...
                  if (file_exists($image_syspath)) {
                      ...
                      $image = '<img src="'.$image_webpath.'" width="128px" height="128px" />';
                  }
              }          
          }
          return $image;
                ...
                if ($dialog) {
                    $html .= '<script type="text/javascript">
                                $(document).ready(function() {
                                    $("#certificate-'.$certif_tool_id.'-link").colorbox({inline:true, width:"1105px", height:"600px", href:"#certificate-'.$certif_tool_id.'-content"});                    
                                });
                              </script>';
                }
                $html .= '<div style="'.($dialog?'display:none':'display:block').'">';
                $html .= '  <div id="certificate-'.$certif_tool_id.'-content" style="padding:10px; background:#fff;position:relative;">';
                $html .= $certificate_info['content'];
                if (!empty($link_export)) {
                    $html .= '<div style="position:absolute;top:42px;left:35px;"><a href="'.$link_export.'" style="text-decoration:none;">'.Display::return_icon('48x48file_pdf.png', get_lang('Export'), array('style'=>'vertical-align:middle;')).' '.get_lang('Export').'</a></div>';
                }
                $html .= '  </div>';
                $html .= '</div>';
          }
          ...
          if (in_array($variable, $this->getPatternsTokenName())) {
              switch ($variable) {
                  ...
                  case '{CertificateMinScore}':
                      $value = $this->certif_min_score;
                      break;
                  case '{UserScore}':
                      $value = $this->user_score;
                      break;
                  case '{SiteUrl}':
