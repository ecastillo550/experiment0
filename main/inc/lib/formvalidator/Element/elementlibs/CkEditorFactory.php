...
class HTML_QuickForm_html_editor extends HTML_QuickForm_textarea
{
    ...
    public function HTML_QuickForm_html_editor($elementName = null, $elementLabel = null, $attributes = null, $config = null)
    {
     ...
     $this->config = $config;
        ...
        if( ! is_null($this->editorName))
        {
            ...
            $this -> fck_editor = new CKeditor($basePath  );
    ...
    {        
        ...
        $result .= '<script type="text/javascript" src="'.$this->basePathCkEditor .'adapters/jquery.js"></script>';
        $result .= '';
        
        $result .= "<script type=\"text/javascript\">";
        $result .= "//<![CDATA[\n";
        $result .= '$( \'textarea[name="'. $this->editorName.'"]\' ).ckeditor(function() {
        }, {
        ...
        //$result .= $this->fck_editor->getScript($this->editorName);
