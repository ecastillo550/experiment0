...
class SortableTable extends HTML_Table {
	...
	public function display () {
			...
			if (count($this->form_actions) > 0)
			{
				$html .= '<script language="JavaScript" type="text/javascript">
																...
																function setCheckbox(value) {
																	var thisvalue = $("input[name=select_all_none]").attr("checked");
																	if (thisvalue == true){
																		value = true;
																	} else {
																		value = false;
		...
		if (!$empty_table)
		{
			...
			if (count($this->form_actions) > 0)
			{
				...
				// if there is more than one action we display a dropdown list with a submit button
				// if there is only one we simply display the button (with hidden form)
				if (count($this->form_actions)>1){
					$html .= '<select name="action">';
					$html .= '<option value="--">'.get_lang('SelectAction').'</option>';
					foreach ($this->form_actions as $action => $label) {
						$html .= '<option value="'.$action.'">'.$label.'</option>';
					}
					$html .= '</select>';
					$button_text = get_lang('Select');
				} else{
					// we have to loop through the array (with only one element) because we do not know what they key or value of the array is
					foreach ($this->form_actions as $action => $label) {
				...
				switch($action)
				{
					case 'delete_courses': 
					case 'delete':	
					$html .= '<button type="submit" class="cancel multiaction" onclick="javascript:if(!confirm('."'".addslashes(api_htmlentities(get_lang("ConfirmYourChoice"),ENT_QUOTES,$charset))."'".')) return false;">'.$button_text.'</button>'; break;
					default: $html .= '&nbsp;&nbsp;<button type="submit" class="save multiaction" onclick="javascript:if(!confirm('."'".addslashes(api_htmlentities(get_lang("ConfirmYourChoice"),ENT_QUOTES,$charset))."'".')) return false;">'.$button_text.'</button>'; break;
				}
	...
	public function return_simple_grid($vibility_options, $hide_navigation) {
		global $charset;
		$empty_table = false;
		$html = '';
		if ($this->get_total_number_of_items() == 0) {
    ...
    // Hide the select box - Bug #6728
		$result[] = '<div style="display:none;"><select name="'.$this->param_prefix.'per_page" onchange="javascript:this.form.submit();">';
	...
	public function set_header ($column, $label, $sortable = true, $th_attributes = null, $td_attributes = null) {
		...
		if ($sortable) {
			$link = '<a href="'.api_get_self().'?'.api_get_cidreq().'&origin='.$origin.'&amp;';
			foreach ($param as $key => $value) {
	...
	public function set_form_actions ($actions, $checkbox_name = 'id') {
		...
		// if there are multiple actions possible on the items of the sortable table 
		// then the first column header should display a checkbox
		if (count($this->form_actions) > 0){
			$this->set_header(0,'<input type="checkbox" value="select_all_none" name="select_all_none" onclick="javascript:setCheckbox();">',false);
		}
