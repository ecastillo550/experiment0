...
class CKEditor
{
	...
	public function editor($name, $value = "", $config = array(), $events = array())
	{
		...
		$out = "<textarea class='ckEditor' name=\"" . $name . "\"" . $attr . ">" . htmlspecialchars($value) . "</textarea>\n";
		if (!$this->initialized) {
			$out .= $this->init();
		}
		...
		if (!empty($_config))
			$js .= "CKEDITOR.replaceAll('ckEditor', ".$this->jsEncode($_config).");";
		else
			$js .= "CKEDITOR.replaceAll('ckEditor');";
