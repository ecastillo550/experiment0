...
class UserManager
{
	...
	public static function create_user($firstName, $lastName, $status, $email, $loginName, $password, $official_code = '', $language = '', $phone = '', $picture_uri = '', $auth_source = PLATFORM_AUTH_SOURCE, $expiration_date = '0000-00-00 00:00:00', $active = 1, $hr_dept_id = 0, $extra = null, $country_code = '', $civility = '') {
		...
		$sql = "INSERT INTO $table_user
                            ...
                            auth_source = '".Database::escape_string('platform')."',
	 ...
	 * @return boolean true if the user information was updated
	 */
	public static function update_user($user_id, $firstname, $lastname, $username, $password = null, $auth_source = null, $email = '', $status = STUDENT, $official_code = '', $phone = '', $picture_uri = '', $expiration_date = '', $active = 1, $creator_id = null, $hr_dept_id = 0, $extra = null, $language = 'english', $country_code = '', $civility = '') {
    ...
    */
	...
	public static function get_user_picture_path_by_id($id, $type = 'none', $preview = false, $anonymous = false) {
		...
		if (empty($picture_filename) && $anonymous) {
                         return array('dir' => $base.'css/'.api_get_setting('stylesheets').'/images/', 'file' => 'unknown.png');
		}
...
*/
	...
	public static function get_extra_fields($from = 0, $number_of_items = 0, $column = 5, $direction = 'ASC', $all_visibility = true) {
		...
		if ($all_visibility == false) {
			$sqlf .= " WHERE field_visible = 1 and field_registration = 1 ";
		}
   ...
   public static function get_picture_user($user_id, $picture_file, $height, $size_picture = 'medium_', $style = '') {
    	...
    	if (file_exists($file)) {
			...
			if (file_exists($file) && !is_dir($file)) {
				...
				switch ($size_picture) {
					...
					case 'medium_' :
						$picture['file'] = api_get_path(WEB_CSS_PATH).api_get_setting('stylesheets').'/images/unknown_50_50.jpg'; break;
					case 'small_' :
						$picture['file'] = api_get_path(WEB_CODE_PATH).'img/unknown.jpg'; break;
					default:
						$picture['file'] = api_get_path(WEB_CODE_PATH).'img/unknown.jpg'; break;
	...
	public static function get_all_user_tags($tag, $field_id = 0, $from = null, $number_of_items = null) {
		...
		$sql = "SELECT u.user_id,u.username,firstname, lastname, email, tag, picture_uri FROM $table_user_tag ut INNER JOIN $table_user_tag_values uv ON (uv.tag_id=ut.id)
				...
				WHERE $where_field tag LIKE '$tag%' AND u.status <> ".ANONYMOUS." ".(api_get_user_id()?" AND u.user_id <> ".  api_get_user_id():" ")." ORDER BY tag";
		...
		$keyword = $tag;
		$sql = "SELECT u.user_id, u.username, firstname, lastname, email, picture_uri FROM $user_table u WHERE u.status <> ".ANONYMOUS." ";
		global $_configuration;
		...
		if (isset ($keyword)) {
				...
				$sql .= " AND (u.firstname LIKE '%".$keyword."%' OR u.lastname LIKE '%".$keyword."%'  OR u.username LIKE '%".$keyword."%'  )";
			}
		$keyword_active = true;
		//only active users
		if ($keyword_active) {
	...
	public static function get_user_manager_name($user_id){
        ...
        public static function send_mail_to_new_user($recipient_name, $email,$subject,$username,$password,$sender_name, $email_admin, $language = ''){
                ...
                $message = str_replace("{siteName}",api_get_setting('siteName'),$message);
                $message = str_replace('{username}',$username, $message); 
                $message = str_replace('{password}',stripslashes($password), $message);                        
                $message = str_replace('{administratorSurname}',api_get_person_name(api_get_setting('administratorName'), api_get_setting('administratorSurname')), $message); 
                $message = str_replace('{administratorTelephone}',api_get_setting('administratorTelephone'), $message); 
                $message = str_replace('{emailAdministrator}',$email_admin, $message); 

                if ($_configuration['multiple_access_urls'] == true) {
                        $access_url_id = api_get_current_access_url_id();
                        if ($access_url_id != -1) {
                                $url = api_get_access_url($access_url_id);
                                $message =  str_replace('{url}',$url['url'], $message); 
                        }
                }
                else {
                ...
                api_mail_html($recipient_name, $email, $subject, $message, $sender_name, $email_admin);  
            
        }
        
         public static function send_mail_to_new_user_for_credit_card_or_installment($recipient_name, $email,$subject,$firstname, $lastname,$username,$password,$programme,$sender_name, $email_admin){
                
                global $language_interface, $_configuration;
                
                $siteUrl = api_get_setting('InstitutionUrl');
                $table_emailtemplate 	= Database::get_main_table(TABLE_MAIN_EMAILTEMPLATES);	
                $sql = "SELECT * FROM $table_emailtemplate WHERE description = 'EmailsRegistrationInCaseCreditCardOrInstallment' AND language= '".$language_interface."'";              
                ...
                $message = str_replace("{Programme}",$programme,$message);
                $message = str_replace("{Institution}",api_get_setting('Institution'),$message); 
                $message = str_replace("{siteName}",api_get_setting('siteName'),$message);
                $message = str_replace("{InstitutionUrl}",$siteUrl,$message);
                $message = str_replace('{username}',$username, $message); 
                $message = str_replace('{password}',stripslashes($password), $message);    
                $message = str_replace("{detailsUrl}",api_get_setting('InstitutionUrl'),$message);                            
                $message = str_replace("{administratorSurname}",api_get_person_name(api_get_setting('administratorName'), api_get_setting('administratorSurname')), $message);   
        ...
        public static function send_mail_to_new_user_for_cheque($recipient_name, $email,$subject,$firstname, $lastname,$username,$password,$programme,$sender_name, $email_admin){
                ...
                $message = str_replace('{password}',stripslashes($password), $message);                 
                $message = str_replace("{siteName}",api_get_setting('siteName'),$message);
                $message = str_replace("{InstitutionUrl}",api_get_setting('InstitutionUrl'),$message);           
                $message = str_replace("{administratorSurname}",api_get_person_name(api_get_setting('administratorName'), api_get_setting('administratorSurname')), $message);                
                ...
                public static function send_mail_to_new_user_for_cheque_to_admin($recipient_name, $email,$subject,$firstname, $lastname,$username,$password,$programme,$sender_name, $email_admin){
                
                global $language_interface, $_configuration;
                
                $siteUrl = api_get_setting('InstitutionUrl');
                $table_emailtemplate 	= Database::get_main_table(TABLE_MAIN_EMAILTEMPLATES);	
                $sql = "SELECT * FROM $table_emailtemplate WHERE description = 'EmailsInCaseOfChequePaymentToAdmin' AND language= '".$language_interface."'";
        ...
        public static function send_mail_add_programmes_for_credit_card_or_installment($recipient_name, $email,$subject,$firstname, $lastname,$programme,$sender_name, $email_admin){
                
                global $language_interface, $_configuration;
                
                $siteUrl = api_get_setting('InstitutionUrl');
                $table_emailtemplate 	= Database::get_main_table(TABLE_MAIN_EMAILTEMPLATES);	
                $sql = "SELECT * FROM $table_emailtemplate WHERE description = 'RegisterUserToProgrammes' AND language= '".$language_interface."'";
        ...
        public static function send_mail_add_programmes_for_cheque($recipient_name, $email,$subject,$firstname, $lastname,$username,$password,$programme,$sender_name, $email_admin){
                
                global $language_interface, $_configuration;
                
                $siteUrl = api_get_setting('InstitutionUrl');
                $table_emailtemplate 	= Database::get_main_table(TABLE_MAIN_EMAILTEMPLATES);	
                $sql = "SELECT * FROM $table_emailtemplate WHERE description = 'RegisterUserToProgrammesforChequePayment' AND language= '".$language_interface."'";
                ...
                public static function send_mail_add_programmes_for_cheque_to_admin($recipient_name, $email,$subject,$firstname, $lastname,$username,$password,$programme,$sender_name, $email_admin){
                
                global $language_interface, $_configuration;
                
                $siteUrl = api_get_setting('InstitutionUrl');
                $table_emailtemplate 	= Database::get_main_table(TABLE_MAIN_EMAILTEMPLATES);	
                $sql = "SELECT * FROM $table_emailtemplate WHERE description = 'RegisterUsertoProgrammesforChequePaymentToAdmin' AND language= '".$language_interface."'";
          ...
          public static function  get_enrolment_and_no_enrolment_rh_list ($enrolment_type) {
            $user_table = Database::get_main_table(TABLE_MAIN_USER);
            $url_table = Database::get_main_table(TABLE_MAIN_ACCESS_URL_REL_USER);
     
            global $_configuration;
            if ($_configuration['multiple_access_urls']==true) {
                $url_id = api_get_current_access_url_id();
                $sql = "SELECT user_id,CONCAT(lastname,'  ',firstname) as 'name' FROM $user_table us INNER JOIN $url_table ur ON us.user_id=ur.user_id WHERE u.access_url_id= '$url_id' status=4 AND default_enrolment=".$enrolment_type."";
                } else {
                $sql = "SELECT user_id,CONCAT(lastname,'  ',firstname) as 'name' FROM $user_table WHERE status=4 AND default_enrolment=".$enrolment_type."";
            }
