	...
	foreach ($a_course_users as $user_id => $o_course_user) {
		if ((isset ($_GET['keyword']) && search_keyword($o_course_user['firstname'], $o_course_user['lastname'], $o_course_user['username'], $o_course_user['official_code'], $_GET['keyword'])) || !isset($_GET['keyword']) || empty($_GET['keyword'])) {
			...
			if (api_is_allowed_to_edit()) {
				...
				$temp[] = $photo;
				if (in_array($user_id, $online_users_list) && $user_id != api_get_user_id()) { // If is online
				  $temp[] = '<center><div class="chat_friend" id="chat_alumno'.$user_id.'"><img src="'.api_get_path(WEB_IMG_PATH).'internet-group-chat22.png" /></div></center>';
				} else {
				  // No display chat image if user is not connected
				  $temp[] = '<center>&nbsp;</center>';
				}
				...
				if (in_array($user_id, $online_users_list) && $user_id != api_get_user_id()) { // If is online
				  $temp[] = '<div class="chat_with_me" id="chat_with_'.$user_id.'"><center><a href="javascript:void(0)" onclick="try{popup_window.close();}catch(e){};popup_window = window.open(\''.api_get_path(WEB_PATH).'main/chat/chat.php?'.api_get_cidreq().'&send_invitation=true&to_user='.$user_id.'\',\'window_chat'.api_get_course_id().'\',config=\'height=500, width=925, left=2, top=2, toolbar=no, menubar=no, scrollbars=yes, resizable=yes, location=no, directories=no, status=no\')"><img src="'.api_get_path(WEB_IMG_PATH).'internet-group-chat22.png" /></a></center></div>';
				} else {
				  // No display chat image if user is not connected
				  //$temp[] = '<center><img src="'.api_get_path(WEB_IMG_PATH).'internet-group-chat22.png" /></center>';
				  $temp[] = '<center>&nbsp;</center>';
				}
...
$table->set_header($header_nr++, get_lang('Chat'), false);
...
if (api_is_allowed_to_edit()) {
	...
	//actions column
	$table->set_header($header_nr++, get_lang('Report'), false);
