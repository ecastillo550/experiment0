...
function get_user_data($from, $number_of_items, $column, $direction) {
	...
	while ($user = Database::fetch_row($res)) {
		...
		$image_array=UserManager::get_user_picture_path_by_id($user[0],'web',false,false);
		$user_profile = UserManager::get_picture_user($user[0], $image_array['file'], 22, 'small_', ' width="22" height="22" ');
		if ($image_array['file']!='') {
			$photo = '<center><a class="thickbox" href="'.$image_array['dir'].$image_array['file'].'" title="'.get_lang('Info').'"  >
						<img src="'.$user_profile['file'].'" style="width:22px" alt="'.api_get_person_name($o_course_user['firstname'], $o_course_user['lastname']).'"  title="'.api_get_person_name($o_course_user['firstname'], $o_course_user['lastname']).'" />
					  </a></center>';
		}
		else{
		...
		$status = "";
		if($user[5] == 0){
			$status = '<div style="text-align:center;"><img src="'.api_get_path(WEB_IMG_PATH).'dialog-error.png" /></div>';
		}
		else{
			if ($user[4] == 1) {
		...
		$users[] = array($user[1], $user[0], $user[2], $user[3], $user[4], $session_name, $status, UserManager::get_user_manager_name($user[7]));
...
$form->addElement('static','additionalactions',null, '<div style="width:570px;float:left;">'.$actions.'</div>');
$form->addElement('static', 'div_float_right', null, '<div style="width:350px;float:left;">');
