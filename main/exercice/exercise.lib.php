    ...
    if ($answerType != HOT_SPOT && $answerType != HOT_SPOT_DELINEATION) { // Question is not of type hotspot
	    ...
	    $questionName = trim($questionName);
            ...
            if ($answerType == MATCHING) {
                $s = "<div id=\"question_title\" class=\"quiz_content_actions\" style=\"width:98%\">";
                echo $s;
                echo $questionName .'</div>';
                if (!empty($questionDescription)) {
                echo '<div style="width:98%" class=\'quiz_content_actions\'><div class=\'media_scroll\'>'.$questionDescription.'</div></div>';
                }
                // Only for preview
                if (isset($_SESSION['is_within_submit']) && $_SESSION['is_within_submit'] == 1) {
                    //   echo '<div class="quiz_content_actions">';
                    echo '<form name="frm_exercise" id="frm_exercise" style="width:98%" class="quiz_content_actions">';
                } else {
                    echo '<form name="frm_exercise" id="frm_exercise" style="width:98%" class="quiz_content_actions">';
                }
            } else {
                if($mediaPosition == 'top' || $mediaPosition == 'nomedia' || empty($questionDescription)){				
                $s = "<div id=\"question_title\" class=\"quiz_content_actions\" style=\"float:left;width:98%\">";
				}
				elseif($mediaPosition == 'right'){				
				$s = "<div id=\"question_title\" class=\"quiz_content_actions\" style=\"float:left;width:50%\"><div class=\"sectscroll\">";
				}
                echo $s;
                echo $questionName . '</div></div>';
            }
            $s = '';
            if (!empty($questionDescription)) {
                if($mediaPosition == 'right' && $answerType != MATCHING){
				$s.="<div style='float:right;'>";
                $s.="<div style='width:400px;height:auto;min-height:280px;border-radius: 5px;
				background-color:#fff;margin-top:5px;-moz-border-radius: 5px;-webkit-border-radius: 5px;border:1px solid #b8b8b6;-webkit-border-radius: 5px;
        -webkit-box-shadow: 0px 0px 7px #cccccc; 
        -moz-box-shadow: 0px 0px 7px #cccccc;'><div class='media_scroll'>";
                $questionDescription = api_parse_tex($questionDescription);
                $s.=$questionDescription;
                $s.="</div></div></div>";
				}
				if($mediaPosition == 'top' || $mediaPosition == 'nomedia' || empty($questionDescription) || $answerType == MATCHING){
				$s.="<div class='quiz_content_actions' style='width:98%;float:left'>";	
				}
				elseif($mediaPosition == 'right'){
        ...
        for ($answerId = 1; $answerId <= $nbrAnswers; $answerId++) {
            ...
            if ($answerType == FILL_IN_BLANKS) {
                ...
                foreach ($matches[0] as $match) {
                    ...
                    if ($answer_len <= 4) {						
                        $size = "2";
                        $temp = str_replace($match, '<input type="text" name="choice[u' . $questionId . '][]" size="' . $size . '" >', $answer);
                        $answer = $temp;
                    } 
...
$(document).ready(function(){';
	  ...
	  $("#q' . $questionId . '-' . $i . '").droppable({
		   //I used this to make only a1 acceptable for q1
            ...
            if ($image_match == 'Y') {
                echo '
			$(this).html("<div style=\"border:1px solid #000;background-color:#fff;height:124px;width:272px;\">"+answer+"</div>");
			';
            } else {
                echo '
			$(this).html("<div style=\"border:1px solid #000;background-color:#fff;height:43px;width:271px;padding-bottom:1px;\">"+answer+"</div>");
			';
            }
            echo '
        ...
        for ($i = 0; $i < count($QA); $i++) {
            echo '<tr><td  width="50%"><div class="question">'. $QA[$i] . '</div></td><td  width="50%"><div id="q'. $questionId . '-' . $i . '" class="' . $destinationbox_css . '"></div></td></tr>';
        }
...
function showFeedback($questionId, $onlyAnswers=false, $origin=false, $current_item = 0, $total_item = 0, $exe_id = 0) {
	...
	if($mediaPosition == 'right'){
		echo '<div class="quiz_content_actions" style="width:98%;">';
  ...
  <tr><td>' . $yourChoice . '</td><td>' . $expectedAnswer . '</td>';
    ...
    if ($answerType == FILL_IN_BLANKS) {
		...
		for ($k = 0; $k < sizeof($feedback_anscorrect); $k++) {
            ...
            if ($feedback_anscorrect[$k] == "Y") {
                $s .= '<td><img src="../img/Right32tr.png">&nbsp;' . get_lang('Right') . '</td><td><span style="font-weight: bold; color: #008000;">' . $feedback_true . '</span></td></tr>';
            } else {
                $s .= '<td><img src="../img/Wrong32tr.png">&nbsp;' . get_lang('Wrong') . '</td><td><span style="font-weight: bold; color: #FF0000;">' . $feedback_false . '</span></td></tr>';
            }
        }
		$s .= '</table></div>';
    }
    if ($answerType == MATCHING) {
        ...
        if ($answer_ok == 'Y' && $answer_wrong == 'N') {
            echo '<td colspan="3" style="border-top: none; padding-top: 0px;"><span style="font-weight: bold; color: #008000;">' . $answerComment_true . '</span></td>';
        } else {
            echo '<td colspan="3" style="border-top: none; padding-top: 0px;"><span style="font-weight: bold; color: #FF0000;">' . $answerComment_false . '</span></td>';
        }
