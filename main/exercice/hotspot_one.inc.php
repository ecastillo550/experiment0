...
if ($from_tpl == 1) {
    $sql =  "SELECT question,description,ponderation,type,level FROM $TBL_QUESTIONS_TEMPLATE WHERE id = " . Database::escape_string($editQuestion);
	$res = Database::query($sql,__FILE__,__LINE__);
	while($row = Database::fetch_array($res)){
		$tpl_question = $row['question'];
		if(str_word_count($tpl_question) == 1){
			$tpl_question = get_lang($tpl_question);
		}
		$tpl_description = $row['description'];
		$tpl_ponderation = $row['ponderation'];
		$tpl_type = $row['type'];
		$tpl_level = $row['level'];
	}

    $sql = "INSERT INTO $TBL_QUESTIONS (question,description,ponderation,type,level) VALUES('".$tpl_question."','".$tpl_description."','".$tpl_ponderation."','".$tpl_type."','".$tpl_level."')";
	...
	$tmp_picpath = api_get_path(SYS_CODE_PATH).'default_course_document/images/';
    copy($tmp_picpath . '/' . $source_picture, $picturePath . '/' . $picture);
    ...
    $sql = "SELECT count(*) AS answers FROM $TBL_REPONSES_TEMPLATE WHERE question_id = " . Database::escape_string($editQuestion);
    $res = Database::query($sql, __FILE__, __LINE__);
    $row = Database::fetch_object($res);
    // Next question order
    $nbrAnswers = $row->answers;
    for ($i = 1; $i <= $nbrAnswers; $i++) {
        $sql = "SELECT * FROM $TBL_REPONSES_TEMPLATE WHERE question_id = " . Database::escape_string($editQuestion) . " AND id = " . $i;
        $result = Database::query($sql, __FILE__, __LINE__);
        while ($row = Database::fetch_array($result)) {
			if(str_word_count($row['answer']) <= 3){				
				$qn_answer[$i] = get_lang($row['answer']);
			}
			else {				
				$qn_answer[$i] = $row['answer'];
			}

			if(str_word_count($row['comment']) <= 3){				
				$qn_comment[$i] = get_lang($row['comment']);
			}
			else {				
				$qn_comment[$i] = $row['comment'];
			}

            $sql = "INSERT INTO $TBL_REPONSES(id,question_id,answer,correct,comment,ponderation,position,hotspot_coordinates,hotspot_type) VALUES(" . $i . "," . Database::escape_string($insert_id) . ",'" . $qn_answer[$i] . "'," . $row['correct'] . ",'" . $qn_comment[$i] . "'," . $row['ponderation'] . "," . $row['position'] . ",'" . Database::escape_string($row['hotspot_coordinates']) . "','" . $row['hotspot_type'] . "')";
                    ...
                    for ($i = 1; $i <= $nbrAnswers; $i++) {
                        ...
                        ?>
                            <tr  class="<?php echo $class; ?> actions">
                                ...
                                <td height="30" align="left" valign="top"><input type="text" name="reponse[<?php echo $i; ?>]" value="<?php echo api_convert_encoding($reponse[$i], ENT_QUOTES, api_get_system_encoding()); ?>" size="15" /></td>
