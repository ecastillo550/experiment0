            ...
            function Removefillup()
	    {
			...
			if(blanks!=null){
				for(i=0 ; i<blanks.length ; i++){
					...
					var blank_str = str.replace("</span></u>","");
					if(blank_str == "[]")
					{
						return;				
					}
...
// $form->addElement('textarea', 'comment[1]', null, 'id="comment[1]" cols="55" rows="1"');
   $form->add_html_editor('comment[1]','', false, false, array('ToolbarSet' => 'TestProposedFeedback', 'Width' => '400px', 'Height' => '40px'));
   $form->addElement('html', '</div>');
...
// $form->addElement('textarea', 'comment[2]', null, 'id="comment[2]" cols="55" rows="1"');
   $form->add_html_editor('comment[2]','', false, false, array('ToolbarSet' => 'TestProposedFeedback', 'Width' => '400px', 'Height' => '40px'));
