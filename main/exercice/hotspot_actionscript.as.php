...
// set vars
$questionId    = Security::remove_XSS($_GET['modifyAnswers']);
...
while ($hotspot = mysql_fetch_assoc($result))
{
	...
	$output .= "&hotspot_".$hotspot['id']."_answer=".mb_convert_encoding($hotspot['answer'], 'UTF-8', 'HTML-ENTITIES');//mb_convert_encoding($hotspot['answer'], 'UTF-8', 'HTML-ENTITIES');//str_replace('&','{amp}',$hotspot['answer']);
	// Square or rectancle	
