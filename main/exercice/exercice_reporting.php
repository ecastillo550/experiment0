    ...
    if ($result_disabled == 0) {
     ...
     if ($is_allowedToEdit || $is_tutor) {
      if ($revised) {
       ...
       echo "<td><a href='exercise_show.php?".api_get_cidreq()."&action=qualify&user=$url_user&dt=$dt&res=$res&id=$id&email=$mailid&exerciseid=$quiz_id'>" . Display :: return_icon('pixel.gif', get_lang('Qualify'),array('class'=>'actionplaceholdericon actionmarkexercise'))."</a></td>";
      }   
      ...
      if ($is_allowedToEdit) {
       echo '<td> <a href="exercice_history.php?'.api_get_cidreq().'&exe_id=' . $id . '">' . Display :: return_icon('pixel.gif', get_lang('ViewHistoryChange'),array('class'=>'actionplaceholdericon actionfootprint')) . '</a></td>';
      }
    } else {
      if ($revised)
