...
if ($origin != 'learnpath') {    
        ...
        if (api_is_course_admin() && api_is_allowed_to_edit()) {
            ...
            echo '<a href="'.api_get_path(WEB_CODE_PATH).'exercice/exercice_scenario.php?scenario=yes&modifyExercise=yes&' . api_get_cidreq() . '&exerciseId='.$exercise_id.'">' . Display::return_icon('pixel.gif', get_lang('Scenario'), array('class' => 'toolactionplaceholdericon toolactionscenario')) . get_lang('Scenario') . '</a>';
        } else {
        ...
        <div class="content-quiz-result-actions" style="position:relative;">
            <?php
                if ($certif_available && $origin != 'learnpath') {                    
                    echo '<a id="certificate-'.$exercise_id.'-link" href="#">'.Display::return_icon('certificate48x48.png', get_lang('GetCertificate'), array('style'=>'position:absolute;top:15px;right:10px;')).'</a>';
...
if ($show_results) {
  ...
  <!--	<div id="question_description" class="scroll_feedback">
			...
			for ($answerId=1;$answerId <= $nbrAnswers;$answerId++) {
				...
				for ($k = 0; $k < sizeof($feedback_anscorrect); $k++) {
					...
					if ($feedback_anscorrect[$k] == "Y") {
						$s .= '<td><img src="../img/Right32tr.png" style="vertical-align:middle;">&nbsp;' . get_lang('Right') . '</td><td><span style="font-weight: bold; color: #008000;">' . $feedback_true . '</span></td></tr>';
					} else {
						$s .= '<td><img src="../img/Wrong32tr.png" style="vertical-align:middle;">&nbsp;' . get_lang('Wrong') . '</td><td><span style="font-weight: bold; color: #FF0000;">' . $feedback_false . '</span></td></tr>';
					}
				}
				$s .= '</table></div>';
				$i++;
		...
		if ($is_allowedToEdit && in_array($origin, array('tracking_course','user_course'))) {
			echo '<tr><td>';							
...
if ($origin!='learnpath' || ($origin == 'learnpath' && isset($_GET['fb_type']))) {
	if ($show_results) {
		echo '<div id="question_score" class="actions" style="min-height:30px;font-weight:bold;position:relative;">';
                if ($certif_available && $origin != 'learnpath') {                   
                   echo '<a id="certificate-'.$exercise_id.'-link" href="#">'.Display::return_icon('certificate48x48.png', get_lang('GetCertificate'), array('style'=>'position:absolute;top:0px;right:10px;')).'</a>';
