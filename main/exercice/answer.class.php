	...
	function read()
	{
		...
		if($_GET['fromTpl'] <> 0){
			$sql="SELECT id,answer,correct,comment,ponderation, position, hotspot_coordinates, hotspot_type, destination FROM
					$TBL_REPONSES_TEMPLATE WHERE question_id ='".Database::escape_string($questionId)."' ORDER BY position";
			...
			{
				$this->id[$i]=$object->id;
				if(str_word_count($object->answer) <= 3){				
					$this->answer[$i] = get_lang($object->answer);
				}
				else {				
				...
				if(str_word_count($object->comment) <= 3){				
					$this->comment[$i] = get_lang($object->comment);
				}
				else {				
