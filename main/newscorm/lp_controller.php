...
<script>
  $(document).ready(function (){
    //$("div.label").attr("style","width: 15%;text-align:left");
    //$("div.row").attr("style","width: 10px; height:10px; text-align:left; position:relative");
...
switch($action)
{
		...
		if(!$lp_found){
			...
			$_SESSION['refresh'] = 1;
                        ...
                        if($_POST['type'] == TOOL_DOCUMENT) {
                            $_POST['title'] = !empty($_POST['title'])?$_POST['title']:get_lang('Empty');
                        }
		...
		//	$_SESSION['oLP']->set_author($author_fixed);
			...
			$_SESSION['oLP']->update_scorm_debug($_REQUEST['enable_debug']);
                        ...
                        if (api_get_setting('enable_certificate') === 'true') {
                            $_SESSION['oLP']->set_certif_template($_REQUEST['certificate_template']);
                            if ($_REQUEST['certif_evaluation_type'] == 1) {
                                $_SESSION['oLP']->set_certif_min_progress($_REQUEST['certificate_min_score']);
                                $_SESSION['oLP']->set_certif_min_score(0);                            
                            } else {
                                $_SESSION['oLP']->set_certif_min_score($_REQUEST['certificate_min_score']);
                                $_SESSION['oLP']->set_certif_min_progress(0);
                            }
                            if (!empty($_REQUEST['certificate_template'])) {
                                $_SESSION['oLP']->save_certificate_item($_REQUEST['certificate_template'], get_lang('Certificate'), '');
                            }                        
                        }
	...
	case 'return_to_course_homepage':
        if(!$lp_found){ error_log('New LP - No learnpath given for stats',0); require('lp_list.php'); }
            ...
            api_session_unregister('oLP');
            api_session_unregister('lpobject');
			if(isset($_REQUEST['learner_view'])){
				$param = "?learner_view=true";
			}
			else {
				$param = '';
			} 
            header('location:'.api_get_path(WEB_COURSE_PATH).api_get_course_path().'/index.php'.$param);
        }
        break; 
    case 'search':
