 ...
 * @author Patrick Cool <patrick.cool@UGent.be>, original author
...
function display_addedresource_link($type, $id, $style='')
{
	...
	switch ($type)
	{
		case 'Agenda':
			...
			echo '<img src="../img/agenda.png" align="middle" /> <a href="../calendar/agenda.php"'.$styling.'>'.$myrow['title']."</a><br />\n";
			break;
		case 'Ad_Valvas':
			...
			$myrow = Database::fetch_array($result);
			echo '<img src="../img/valves.png" align="middle" /> <a href="../announcements/announcements.php"'.$styling.'>'.$myrow['title']."</a><br />\n";
			break;
		case 'Link':
...
function display_addedresource_link_in_learnpath($type, $id, $completed, $id_in_path, $builder, $icon, $level = 0)
{
	...
	switch ($type)
	{
		...
		case "Agenda":
			...
			if ($icon == 'icon') { echo "<img src='../img/agenda.png' align=\"absmiddle\" alt='agenda'>"; }
			if ($builder != 'builder')
			{
		...
		case "Ad_Valvas":
			...
			if ($icon == 'icon') { echo "<img src='../img/valves.png' align=\"absmiddle\" alt='ad valvas'>"; }
			if ($builder != 'builder')
			{
...
function display_added_resources($type, $id, $style='')
{
	...
	$arr_icons=array('Agenda'=>'../img/agenda.png', 'Ad Valvas'=>'../img/valves.png', 'Link'=>'../img/links.gif', 'Exercise'=>'../img/quiz.gif' );
...
function rl_get_html_resource_link($course_code, $type, $id, $style='', $new_window=true)
{
	...
	switch ($type)
	{
		case TOOL_CALENDAR_EVENT:
			...
			$result = Database::query("SELECT * FROM $TABLEAGENDA WHERE id=$id",__FILE__,__LINE__);
			$myrow = Database::fetch_array($result);
			$output = '<img src="../img/agenda.png" align="middle" /> <a href="../calendar/agenda.php"'.$styling.' '.$target.'>'.$myrow['title']."</a><br />\n";
			break;
		case TOOL_ANNOUNCEMENT:
			$tbl_announcement = Database::get_course_table(TABLE_ANNOUNCEMENT,$_course['database']);
			$result = Database::query("SELECT * FROM $tbl_announcement WHERE id=$id",__FILE__,__LINE__);
			$myrow = Database::fetch_array($result);
			$output = '<img src="../img/valves.png" align="middle" /> <a href="../announcements/announcements.php"'.$styling.' '.$target.'>'.$myrow['title']."</a><br />\n";
			break;
		case TOOL_LINK:
...
function rl_get_resource_link_for_learnpath($course_code, $learnpath_id, $id_in_path)
{
	...
	switch ($type)
	{
		...
		case TOOL_CALENDAR_EVENT:
			$link .= $main_dir_path.'calendar/agenda.php?origin='.$origin.'&agenda_id='.$id.'&'.api_get_cidreq();
			break;

		case TOOL_ANNOUNCEMENT:
			$link .= $main_dir_path.'announcements/announcements.php?origin='.$origin.'&ann_id='.$id.'&'.api_get_cidreq();
			break;
		...
		case TOOL_QUIZ:
			if(!empty($id))
			{
				...
				$link .= $main_dir_path.'exercice/exercice_submit.php?lp_init=1&origin='.$origin.'&learnpath_id='.$learnpath_id.'&learnpath_item_id='.$id_in_path.'&exerciseId='.$id.'&'.api_get_cidreq();
			}
			break;
		...
		case "hotpotatoes": //lowercase because of strtolower above
			...
			$link .= $main_dir_path.'exercice/showinframes.php?file='.$path.'' .
					...
					'&learnpath_id='.$learnpath_id.'&learnpath_item_id='.$id_in_path.'&'.api_get_cidreq();
			break;

		case TOOL_FORUM:
			$link .= $main_dir_path.'forum/viewforum.php?forum='.$id.'&lp=true&origin=learnpath&'.api_get_cidreq();
			break;
		...
		case TOOL_THREAD:  //forum post
			...
			if(!empty($id))
			{
				...
				$link .= $main_dir_path.'forum/viewthread.php?origin=learnpath&thread='.$id.'' .
						'&forum='.$myrow['forum_id'].'&lp=true&'.api_get_cidreq();
			}
			break;
		...
		case TOOL_DOCUMENT:
			...
			$docurl=str_replace('%2F','/',$myrow['path']);
   ...
   if (api_is_allowed_to_edit ()) {
			  $link .= $main_dir_path.'survey/preview.php?survey_id='.$id.'&origin=learnpath&'.api_get_cidreq();
   } else {
    $uid = api_get_user_id();
			 $tbl_survey_invitation = Database::get_course_table(TABLE_SURVEY_INVITATION);
    $sql = "SELECT invitation_code FROM ".$tbl_survey_invitation." WHERE user='".$uid."'";
    $rs = Database::query($sql, __FILE__, __LINE__);
    $row = Database::fetch_array($rs);
    $invitation_code = $row['invitation_code'];
    // Survey link
			 $link .= $main_dir_path.'survey/fillsurvey.php?course='.api_get_course_id().'&user_id='.$uid.'&origin=learnpath&'.api_get_cidreq().'&&invitationcode='.$invitation_code;
   }
