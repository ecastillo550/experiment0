...
class learnpath {
    ...
    public $lp_interface; // The current course interfaz for the LP
    ...
    function learnpath($course, $lp_id, $user_id) {
        ...
        if (empty($lp_id)) {
            ...
            if (Database::num_rows($res) > 0) {
                ...
                $this->certif_template = $row['certif_template'];
                $this->certif_min_score = $row['certif_min_score'];
                $this->certif_min_progress = $row['certif_min_progress'];
    ...
    /**
     * Save a certificate as lp item  
     * @param   int     Certificate reference id
     * @param   string  Item title
     * @param   string  Description title
     * @return  int     Last insert id
     */
    function save_certificate_item($ref_id, $title, $description) {
        ...
        $certif_item_id = 0;
        // check if exists one certificate
        $check = Database::query("SELECT id FROM $tbl_lp_item WHERE item_type='certificate' AND lp_id='".intval($this->lp_id)."'");
        if (Database::num_rows($check) > 0) {
            // update
            $row_certif = Database::fetch_array($check, 'ASSOC');
            $certif_item_id = $row_certif['id'];
            Database::query("UPDATE $tbl_lp_item SET 
                                title = '$title',
                                description = '$description',                    
                                ref = '".intval($ref_id)."'
                             WHERE id = $certif_item_id       
                            ");
        }
        else {
            Database::query("INSERT INTO $tbl_lp_item SET 
                                lp_id = {$this->lp_id},
                                title = '$title',
                                description = '$description',
                                ...
                                ref = '".intval($ref_id)."'    
                            ");
            $certif_item_id = Database::insert_id();
        }
        return $certif_item_id;
    }
    ...
    function add_lp($course, $name, $description = '', $learnpath = 'guess', $origin = 'zip', $zipname = '', $keyword = '') {
        ...
        // the ecommerceCAtalog array is to be used for ecommerce catatlog table
        $i = 0;
        ...
        $ecommerceCatalog['title'] = $name;
        $ecommerceCatalog['description'] = $description;
        $ecommerceCatalog['lp_id'] = $response;
    ...
    function get_current_item_id() {
        ...
        /*if ($this->items[$this->current]->get_type() == 'certificate') {
            $this->current = $this->get_next_item_id();
        }*/
    ...
    function get_toc() {
        ...
        foreach ($this->ordered_items as $item_id) {
            ...
            $toc[] = array(
                ...
                'item_type' => $this->items[$item_id]->get_type()
            );
        }
        if ($this->debug > 2) {
    ...
    function get_flat_ordered_items_list($lp, $parent = 0) {
        ...
        $certif_item = array();
        while ($row = Database :: fetch_array($res)) {
            // order certificate item to last one
            if ($row['item_type'] == 'certificate') {
                $certif_item = $row['id'];
                continue;
            }
            $sublist = self::get_flat_ordered_items_list($lp, $row['id']);
            $list[] = $row['id'];            
            foreach ($sublist as $item) {                
                $list[] = $item;
            }
        }
        if (!empty($certif_item)) { $list[] = $certif_item; }
        return $list;
    }
    ...
    function get_html_toc() {
        ...
        $html = '<div class="actions" style="height:655px;overflow:auto;"><div class="scorm_title"><div class="scorm_title_text">' . Security::remove_XSS(api_convert_encoding($this->get_name(), $charset, api_get_system_encoding())) . '</div></div>';
        // build, display
    ...
    /**
     * Sets the certificate template and save
     * @param   int     Certificate template id
     * @return  void
     */
    function set_certif_template($certif_template) {
        $this->certif_template = intval($certif_template);
        $lp_table = Database :: get_course_table(TABLE_LP_MAIN);
        Database::query("UPDATE $lp_table SET certif_template = {$this->certif_template} WHERE id = {$this->get_id()}");        
    }
    
    /**
     * Sets the certificate min score and save
     * @param   float   Certificate minimun score
     * @return  void
     */
    function set_certif_min_score($certif_min_score) {
        $this->certif_min_score = floatval($certif_min_score);
        $lp_table = Database :: get_course_table(TABLE_LP_MAIN);
        Database::query("UPDATE $lp_table SET certif_min_score = '{$this->certif_min_score}' WHERE id = {$this->get_id()}");        
    }
    
    /**
     * Sets the certificate min progress and save
     * @param   float   Certificate minimum progress
     * @return  void
     */
    function set_certif_min_progress($certif_min_progress) {
        $this->certif_min_progress = floatval($certif_min_progress);
        $lp_table = Database :: get_course_table(TABLE_LP_MAIN);
        Database::query("UPDATE $lp_table SET certif_min_progress = '{$this->certif_min_progress}' WHERE id = {$this->get_id()}");        
    }
    ...
    function overview() {
        ...
        for ($i = 0; $i < count($arrLP); $i++) {
            ...
            if ($item_type == "link") {
                ...
                $div_class = "class='forumstage'";
            } elseif ($item_type == "student_publication") {
                $img_type = "../img/navigation/assignement_48.png";
            }
            $div_style = 'style="background-image:url(' . $img_type . ');background-repeat:no-repeat;width:100px;height:100px;background-position: center center;"';
            $return .= '<div class="imageBox" id="imageBox' . $id . '">
                        <div class="imageBox_theImage" style="background-image:url(\'../img/scenario.png\');background-repeat:no-repeat;background-position:left;">
                            <div '.(($div_class == "") ? $div_style : $div_class).' >
...
<?php 
        ...
        if (is_numeric($item_id)) {
            ...
            switch ($row['item_type']) {
                ...
                case TOOL_LINK :
                    ...
                    //	$return .= $this->display_manipulate($item_id, $row['item_type']);
                    $return .= "<a href='" . $preview_link . "'>".Display::return_icon('pixel.gif',get_lang('preview'),array('class'=>'mediaactionplaceholdericon mediaactionlink','style'=>'float:right;', 'alt'=>get_lang('preview')))."</a><br/><br/>";
                ...
                case TOOL_FORUM :
                    //	$return .= $this->display_manipulate($item_id, $row['item_type']);
                    $return .= "<a href='" . $preview_link . "'>".Display::return_icon('pixel.gif',get_lang('preview'),array('class'=>'mediaactionplaceholdericon mediaactionforum','style'=>'float:right;','alt'=> get_lang('preview')))."</a><br/><br/>";
    ...
    function get_documents() {
        ...
        $sql_doc = "
					...
					FROM " . $tbl_doc . "
					WHERE
						path NOT LIKE '%_DELETED_%' AND path not LIKE '/video' AND path not LIKE '/audio'
        ...
        $return = '<a href="javascript:void(0)" id="document_tool_id"  class="big_button four_buttons rounded grey_border select_doc_button">' . $document_lang_var . '</a>';
        ...
        $return .= '<div id="document_tool_conten_id" class="gradient rounded_10 grey_border" style="display:none;">' .
                '<strong>' . $doc_lang_var . '</strong><br/><br/>';
     ...
     * @return unknown
     */
    function display_document($id, $show_title = true, $iframe = true, $edit_link = false) {
    ...
    function display_item_form($item_type, $title = '', $action = 'add', $id = 0, $extra_info = 'new') {
        ...
        if (($item_type == 'asset' || $item_type == 'sco') && ($extension == 'html' || $extension == 'htm')) {
            ...
            $form->addElement('html_editor', 'content_lp', '', null, array('ToolbarSet' => 'LearningPathDocuments', 'Width' => '100%', 'Height' => '900', 'FullPage' => true));
    ...
    function display_document_form($action = 'add', $id = 0, $extra_info = 'new') {
      ...
      AND prop.visibility = 1 AND doc.id = '" . Database::escape_string(Security::remove_XSS($_GET['tplid'])) . "'";
        ...
        //$form->addRule('title', get_lang('RequiredField'), 'required');
        $defaults["description"] = api_convert_encoding($item_description, $charset, $this->encoding);
        $form->addElement('html', $return);
        if ($action != 'move') {
            $form->addElement('html', '<div style="padding-right:1px;text-align:left;">');
            ...
            if (!$no_display_add) {
                if (($extra_info == 'new' || $extra_info['item_type'] == TOOL_DOCUMENT || $_GET['edit'] == 'true')) {
                    ...
                    if (!$no_display_edit_textarea) {
                        ...
                        $editor_config = array('ToolbarSet' => (api_is_allowed_to_edit() ? 'Documents' : 'DocumentsStudent'), 'Width' => '100%', 'Height' => '900', 'FullPage' => true,
                        ...
                        $form->addElement('html', '<div class="posbutton">');
    ...
    function get_exercises() {
        ...
        $return = '<a href="javascript:void(0)" id="quiz_tool_id" class="big_button four_buttons rounded grey_border select_quiz_button">' . $quiz_lang_var . '</a>';

        $return .= '<div id="quiz_content_id" class="gradient rounded_10 grey_border" style="display:none;">' .
                '<strong>' . $exercice_lang_var . '</strong><br/><br/>';
    ...
    function get_links() {
        ...
        $return = '<a href="javascript:void(0)" id="link_tool_id" class="big_button four_buttons rounded grey_border link_button">' . $links_lang_var . '</a>';
        ...
        $return .= '<div id="link_content_id" class="gradient rounded_10 grey_border" style="display:none;">' .
                '<strong>' . $links_lang_var . '</strong><br/><br/>';
    ...
    function get_forums() {
        ...
        $return = '<a href="javascript:void(0)" id="forum_tool_id" class="big_button four_buttons rounded grey_border forum_button">' . $forum_lang_var . '</a>';

        $return .= '<div id="forum_content_id" class="gradient rounded_10 grey_border" style="display:none;">' .
                '<strong >' . $forum_lang_var . '</strong><br/><br/>' ;
        ...
        foreach ($a_forums as $forum) {
            ...
            if (!empty($forum['forum_id'])) {
			    ...
			    <a href="' . api_get_self() . '?cidReq=' . Security :: remove_XSS($_GET['cidReq']) . '&amp;action=add_item&amp;type=' . TOOL_FORUM . '&amp;forum_id=' . $forum['forum_id'] . '&amp;lp_id=' . $this->lp_id . '" style="vertical-align:middle">' . api_convert_encoding($forum['forum_title'], $charset, api_get_system_encoding()) . '</a>
    ...
    function get_survey() {
        ...
        $return = '<a href="javascript:void(0)" id="survey_tool_id" class="big_button four_buttons rounded grey_border survey_button">' . $survey_lang_var . '</a>';

        $return .= '<div id="survey_content_id" class="rounded_10 grey_border" style="display:none;">' .
                '<strong>' . $survey_lang_var . '</strong>';
    ...
    function get_student_publication() {
        ...
        $return = '<a href="javascript:void(0)" id="work_tool_id" class="big_button four_buttons rounded grey_border assignement_button">' . $sp_lang_var . '</a>';
        ...
        $return .= '<div id="work_content_id" class="gradient rounded_10 grey_border" style="display:none;">' .
                '<strong>' . $assignments_lang_var . '</strong><br/><br/>';
