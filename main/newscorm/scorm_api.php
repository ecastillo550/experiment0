...
// Correct content-type, this will serve javascript code, so it should be served as such
...
var lms_logs=1; //debug log level for LMS actions. 0=none, 1=light, 2=a lot, 3=all
...
function APIobject() {
  ...
  this.void_save_asset = lms_void_save_asset;
}
...
olms.lesson_location = '<?php if(!api_is_anonymous()) echo $oItem->get_lesson_location();?>';
...
function savedata(origin) {
    ...
    if ((olms.lesson_status != 'completed') && (olms.lesson_status != 'passed') && (olms.mastery_score >=0) && (olms.score >= olms.mastery_score)) {
        // These lines are commented because if the score is saved after each questio, passed status is given and the item is not writable anymore
 ...
 */
function lms_void_save_asset(myscore,mymax)
{
	logit_lms('lms_save_asset',2);
	olms.score = myscore;
...
{
	refreshProgressBar(nbr_complete, nbr_total, mode);
	return true;
...
*/
 ...
 // The cidReq is now very need
	var mysrc = 'lp_controller.php?action=content&lp_id='+olms.lms_lp_id+'&item_id='+next_item+'&'+'cidReq='+'<?php echo $oLP->cidReq; ?>';
	...
	<?php } else { ?>
                        ...
                        if (olms.lms_item_type == "certificate") {                        
                            $.ajax({
                                type: "POST",
                                url: "lp_ajax.php?action=display_certificate&lp_id="+olms.lms_lp_id,
                                success: function(data) {
                                    cont_f.contents().find('html').html(data);
                                }
                            });
                        } else {
   ...
   // (5) switch audio recorder
     ...
     // (6) 
        $.ajax({
		type: "GET",
		url: "lp_ajax_current_item_title?lp_item_id="+next_item,
		success: function(tmp_data) {
                    $("div.title-resource-course").text(tmp_data);
                }
        });
	...
	*/
    ...
    if (is_interactions=='true')  {
        ...
        for (i in olms.interactions) {
        	...
        	params['interact['+i+']'] = interact_string;
        }
...
function xajax_save_objectives(lms_lp_id,lms_user_id,lms_view_id,lms_item_id,item_objectives) {
    ...
    */
    ...
    for (i in item_objectives){
        ...
        params['objectives['+i+']'] = obj_string;
    }
