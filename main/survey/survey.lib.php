...
class survey_manager
{
	...
	function get_survey($survey_id,$shared=0,$course_code='')
	{
        ...
        /*  this function return nums of surveys by id  of the survey
         *  @param integer $survey_id the id of the survey
         *  @return array
         */
        
        function get_ids_survey($survey_id, $learner = false)
        {   
            // getting all the course information
            ($learner) ?  $_course = CourseManager::get_course_information($_GET['course']) : '' ;
           

            $table_survey_question = ($learner) ? (Database :: get_course_table(TABLE_SURVEY_QUESTION, $_course['db_name'])) : (Database :: get_course_table(TABLE_SURVEY_QUESTION));
        ...
        /*  this function return all values of one survey
         *  @param integer $survey_id the id of the survey
         *  @param array/integer $paged_questions the id of the survey
         *  @return array
         */
        
        function get_items_survey ( $survey_id, $paged_questions, $show = null, $learner = false)
        {                      
           ($learner) ?  $_course = CourseManager::get_course_information($_GET['course']) : '' ;
            
            
            $table_survey_question 			= ($learner) ? (Database :: get_course_table(TABLE_SURVEY_QUESTION, $_course['db_name'])) : (Database :: get_course_table(TABLE_SURVEY_QUESTION)) ;
            $table_survey_question_option               = ($learner) ? (Database :: get_course_table(TABLE_SURVEY_QUESTION_OPTION, $_course['db_name'])) : (Database :: get_course_table(TABLE_SURVEY_QUESTION_OPTION)) ;
            ...
            AND survey_question.question_id =  ".(($show != null) ? $paged_questions[$show]: $paged_questions )."
            ORDER BY survey_question.sort, survey_question_option.sort ASC";
            
            
             
           //AND survey_question.question_id IN (".implode(',', $paged_questions).")
	...
	function store_survey($values)
	{
		...
		/*	$sql = 'SELECT 1 FROM '.$table_survey.' WHERE code="'.Database::escape_string($values['survey_code']).'" AND lang="'.Database::escape_string($values['survey_language']).'"';
			...
			$sql = "INSERT INTO $table_survey (code, title, subtitle, author, lang, avail_from, avail_till, is_shared, template, intro, surveythanks, creation_date, anonymous".$additional['columns'].",question_per_page, session_id) VALUES (
						...
						'".Database::escape_string($values['anonymous'])."'".$additional['values'].",
                                                '".Database::escape_string($values['question_per_page'])."',
		...
		else
		{
			...
			$sql = "UPDATE $table_survey SET
							...
							Anonymous	= '".Database::escape_string($values['anonymous'])."'".$additionalsets.",
                                                        question_per_page		= '".Database::escape_string($values['allinone'])."'    
...
//		$this->html .= '<textarea name="question" id="question" class="focus" style="width: 600px; height: 50px;">'.Security::remove_XSS(stripslashes($form_content['question'])).'</textarea>';
        $this->html .=          api_return_html_area('question',$form_content['question'], '', '', null, array('ToolbarSet' => 'Messages', 'Width' => '600', 'Height' => '100'));
		$this->html .= '		</div>';
		$this->html .= '	</div>';
...
class yesno extends question
{
	...
	function create_form($form_content)
	{
		...
		$this->html .= '	<div class="row">';
		$this->html .= '		<div class="label">';
		$this->html .= 				get_lang('AnswerOptions').'&nbsp;&nbsp; <br><br> <label for="answers[0]">1</label>';
		$this->html .= '		</div> <br><br>';
		$this->html .= '		<div class="formw">';
//		$this->html .= '		<textarea name="answers[0]" id="answers[0]" style="width: 600px; height: 50px;">'.stripslashes($form_content['answers'][0]).'</textarea>';
        $this->html .=          api_return_html_area('answers[0]',$form_content['answers'][0], '', '', null, array('ToolbarSet' => 'Messages', 'Width' => '600', 'Height' => '100'));
...
//		$this->html .= '		<textarea name="answers[1]" id="answers[1]" style="width: 600px; height: 50px;">'.stripslashes($form_content['answers'][1]).'</textarea>';
        $this->html .=          api_return_html_area('answers[1]',$form_content['answers'][1], '', '', null, array('ToolbarSet' => 'Messages', 'Width' => '600', 'Height' => '100'));
		$this->html .= '		</div>';
		$this->html .= '	</div>';
	}
	...
	{                    
		foreach ($form_content['options'] as $key=>$value)
		{
                ...
                echo '<div class="survey_question_wrapper" style= "height : auto">';
...
class multiplechoice extends question
{
	...
	function create_form($form_content)
	{
		...
		$this->html .= '		</div>';
		$this->html .= '		<div class="formw">';
//        <textarea name="answers['.$key.']" id="answers['.$key.']" style="width: 600px; height: 50px;">'.api_html_entity_decode(stripslashes($form_content['answers'][$key]), ENT_QUOTES, $charset).'</textarea>';
        $this->html .=          api_return_html_area('answers['.$key.']',$value, '', '', null, array('ToolbarSet' => 'Messages', 'Width' => '600', 'Height' => '100'));
...
//          <textarea name="question'.$form_content['question_id'].'" id="textarea" style="width: 800px; height: 130px;">'.$content.'</textarea>';
        echo api_return_html_area('question'.$form_content['question_id'],$content, '', '', null, array('ToolbarSet' => 'Messages', 'Width' => '800', 'Height' => '150'));
		echo '</div>';
		echo '</div>';
	}
}
...
class SurveyUtil {
	...
	//	echo '<a href="reporting.php?survey_id='.Security::remove_XSS($_GET['survey_id']).'">'.Display::return_icon('back.png',get_lang('BackTo').' '.get_lang('ReportingOverview')).' '.get_lang('BackTo').' '.get_lang('ReportingOverview').'</a>';
		...
		if ($_GET['question'] <> 0)
		{
			echo '<a style="padding-right: 0px;margin-right: 0px;" href="reporting.php?'.api_get_cidreq().'&amp;action='.Security::remove_XSS($_GET['action']).'&amp;survey_id='.Security::remove_XSS($_GET['survey_id']).'&amp;question='.Security::remove_XSS($offset-1).'">'.Display::return_icon('pixel.gif',get_lang('PreviousQuestion'),array('align'=>'middle','class'=>'actionplaceholdericon actionprev_navigation')).' '.get_lang('PreviousQuestion').'</a>';
		}
		else
		{
		...
		{
			echo '<a style="padding-right: 0px;margin-right: 0px;" href="reporting.php?'.api_get_cidreq().'&amp;action='.Security::remove_XSS($_GET['action']).'&amp;survey_id='.Security::remove_XSS($_GET['survey_id']).'&amp;question='.Security::remove_XSS($offset+1).'">'.get_lang('NextQuestion').' '.Display::return_icon('pixel.gif',get_lang('NextQuestion'),array('align'=>'middle','class'=>'actionplaceholdericon actionnext_navigation')).'</a>';
		}
		else
		{
		...
		echo '</div><br/>';
		echo '<div id="sectiontitle_survey">'.$question['survey_question'].'</div>';
		...
		else
		{
			...
			if(is_array($options)){
				foreach ($options as $key=>$value)
				{
					...
					echo '		</td>';
					echo '	</tr>';
					$total_absolute += $absolute_number;
				}
			}
			// displaying the table: footer (totals)
			echo '	<tr>';
			echo '		<td style="border-top:1px solid black;"><b>'.get_lang('Total').'</b></td>';
			echo '		<td style="border-top:1px solid black;" align="right"><b>'.$total_absolute.'</b></td>';
	...
	function display_question_report_score($survey_data, $question, $offset)
	{
		...
		foreach ($options as $key=>$value)
		{
			...
			echo '		<td style="border-top:1px solid black"><b>'.get_lang('Total').'</b></td>';
			echo '		<td style="border-top:1px solid black">&nbsp;</td>';
			echo '		<td style="border-top:1px solid black"><b>'.$total_absolute.'</b></td>';
			echo '		<td style="border-top:1px solid black">&nbsp;</td>';
			echo '		<td style="border-top:1px solid black">&nbsp;</td>';
			echo '	</tr>';
	...
	function survey_list_user($user_id)
	{
		...
		while ($row = Database::fetch_array($result,'ASSOC')) {
			...
			if ($row['answered'] == 0)
			{
				echo '<td><a href="fillsurvey'. $allinone .'.php?course='.$_course['sysCode'].'&amp;invitationcode='.$row['invitation_code'].'&amp;cidReq='.$_course['sysCode'].'">'.$row['title'].'</a></td>';
			}
			else
			{
			...
			if ($link_add===true && $link_available===true) {
				echo '<tr><td colspan  = 3 ><a href="fillsurvey'. $allinone .'.php?user_id='.api_get_user_id().'&amp;course='.$_course['sysCode'].'&amp;invitationcode='.$row['invitation_code'].'&amp;cidReq='.$_course['sysCode'].'">'.get_lang('CompleteTheSurveysQuestions').'</a></td><td></td></tr>';
			}
		}
		echo '</table>';
	}
