...
function check_extension($extension_name,$return_success='Yes',$return_failure='No',$optional=false)
{
	...
	else
	{
		...
		{
			return '<strong>'.$return_failure.'</strong>';
		}
	}
}
...
function check_php_setting($php_setting, $recommended_value, $return_success=false, $return_failure=false)
{
	...
	{
		return '<strong>'.$current_php_value.' '.$return_failure.'</strong>';
	}
}
/**
...
function check_writable($folder)
{
	...
	{
		return '<strong>'.get_lang('NotWritable').'</strong>';
	}
}
...
*/
function display_language_selection_box($name = 'language_list', $default_language='')
{
	//get language list
	...
	if (empty($default_language)){
		$default_language = get_client_language($language_list);
	}
	...
	//$language_to_display = array('asturian','english','italian','french', 'dutch','slovenian','slovenian_unicode','spanish');

	if (is_array($language_to_display)){
...
/**
 * This function checks the language interface of the browser and return it
 * @author Dokeos, 2011
 */
function get_client_language($language_list){
	$language_translation_array = array(
		'ar' 	=> 'arabic',
		'ast' 	=> 'asturian',
		'bg' 	=> 'bulgarian',
		'bs' 	=> 'bosnian',
	...
	if (array_key_exists($client_language, $language_translation_array)){
		return $language_translation_array[$client_language];
	} else {
		return 'english';
	}
}
...
function display_requirements($installType, $badUpdatePath, $updatePath='', $update_from_version_8=array(), $update_from_version_6=array(), $update_from_version_20=array())
{
	...
	echo '<table class="requirements">
			<tr>
				...
				<td class="requirements-value">';
					if (phpversion() < '5.0')
					{
						echo '<strong>'.get_lang('PHPVersionError').'</strong>';
					}
					else
					{
	...
	echo '<table class="requirements">
			<tr>
				<td class="requirements-item">dokeos/main/install/</td>
				<td class="requirements-value">'.check_writable('install/').'</td>
			</tr>
			<tr>
			...
			<tr>
				<td class="requirements-item">dokeos/main/upload/modules/</td>
				<td class="requirements-value">'.check_writable('upload/modules/').'</td>
			</tr>
			<tr>
	...
	if($installType == 'update' && (empty($updatePath) || $badUpdatePath))
	{
		...
		?>
			<table border="0" style="width: 100%;" cellpadding="5" align="left">
			<tr>
	...
	else
	{
		...
		if(!is_writable('../upload/modules')) {
			$notwritable[] = realpath($curdir.'/../upload/modules');
			@chmod('../upload/modules', $perm);
		}
		...
		<button type="submit" name="step1" class="previous_link" onclick="window.location='index.php';return false;" value="&lt; <?php echo get_lang('Previous'); ?>" >&nbsp;&nbsp;<?php echo get_lang('Previous'); ?></button>
	...
	<table><tr><td>
		...
		<table width="100%">
			<tr>
				<td></td>
				<td align="left">
					<button type="submit" class="previous_link" name="step1" value="&lt; <?php echo get_lang('Previous'); ?>" >&nbsp;&nbsp;<?php echo get_lang('Previous'); ?></button>
					<input type="hidden" name="is_executable" id="is_executable" value="-" />
...
*/
function display_database_parameter($install_type, $parameter_name, $form_field_name, $parameter_value, $extra_notice, $display_when_update = true, $tr_attribute='', $attributes=array())
{
	...
	else
	{
		...
		echo '<td><input type="'.$inputtype.'" size="'.DATABASE_FORM_FIELD_DISPLAY_LENGTH.'" maxlength="'.$maxlength.'" name="'.$form_field_name.'" id="'.$form_field_name.'" value="'.htmlentities($parameter_value).'" '.$attr.'/>'."</td>\n";
...
function display_database_settings_form($installType, $dbHostForm, $dbUsernameForm, $dbPassForm, $dbPrefixForm, $enableTrackingForm, $singleDbForm, $dbNameForm, $dbStatsForm, $dbScormForm, $dbUserForm)
{
	...
	<tr>
	  ...
	  <td width="30%"><input type="hidden" name="dbHostForm" id="dbHostForm" value="<?php echo htmlentities($dbHostForm); ?>" /><?php echo $dbHostForm; ?></td>
	  <td width="30%">&nbsp;</td>
	  <?php else: ?>
	  <td width="30%"><input type="text" size="25" maxlength="50" name="dbHostForm" value="<?php echo htmlentities($dbHostForm); ?>" id="dbHostForm" /></td>
	...
	display_database_parameter($installType, get_lang('DBPassword'), 'dbPassForm', $dbPassForm, $example_password,'','',array('onkeyup'=>'javascript:check_mysql_connection();'));
	//database prefix
	...
	<tr>
		<td valign="top"><button type="submit" class="login_link" name="step3" value="<?php echo get_lang('CheckDatabaseConnection'); ?>" id="checkdatabaseconnection" onclick="javascript:check_mysql_connection();return false;">&nbsp;&nbsp;<?php echo get_lang('CheckDatabaseConnection'); ?></button></td>
		<?php $dbConnect = test_db_connect ($dbHostForm, $dbUsernameForm, $dbPassForm, $singleDbForm, $dbPrefixForm); ?>
		<td cosspan="2">
			<div id="connection_feedback"></div>
		</td>
	</tr>
	<tr>
	  <td><br/><br/><br/><br/>
	  <button type="submit" name="step2" class="previous_link" value="&lt; <?php echo get_lang('Previous'); ?>" >&nbsp;&nbsp;<?php echo get_lang('Previous'); ?></button></td>
	  <td><br/><br/><br/><br/>&nbsp;</td>
	  <td align="right"><br/><br/><br/><br/><br/>
	  <input type="hidden" name="is_executable" id="is_executable" value="-" />
	  <button type="submit" class="next" name="step4" value="<?php echo get_lang('Next'); ?> &gt;" id="submitbuttonstep4" disabled="disabled"/><?php echo get_lang('Next'); ?></button></td>
	</tr>
	</table>
	<?php
}
...
function display_configuration_settings_form($installType, $urlForm, $languageForm, $emailForm, $adminFirstName, $adminLastName, $adminPhoneForm, $campusForm, $institutionForm, $institutionUrlForm, $encryptPassForm, $allowSelfReg, $allowSelfRegProf, $loginForm, $passForm)
{
	...
	echo "<tr>\n";
	echo '<td>'.get_lang('DokeosURL').' ('.get_lang('ThisFieldIsRequired').")&nbsp;&nbsp;</td>\n";
	...
	<tr>
	  <td><button type="submit" class="previous_link" name="step3" value="&lt; <?php echo get_lang('Previous'); ?>" />&nbsp;&nbsp;<?php echo get_lang('Previous'); ?></button></td>
