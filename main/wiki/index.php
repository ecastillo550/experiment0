...
var selection = "";
var selected = String(oEditor.EditorWindow.getSelection());  
...
if ($_GET['action']=='delete')
{
	...
	if(api_is_allowed_to_edit(false,true) || api_is_platform_admin())
	{
		...
		if($page=="index")
		{
			Display::display_warning_message(get_lang('WarningDeleteMainPage'),false,true);
		}
		...
		if ($_GET['delete'] == 'yes')
		{
			...
			check_emailcue(0, 'E');
	  		...
	  		Display::display_confirmation_message(get_lang('WikiPageDeleted'), false, true);
		}	else {
			Display::display_warning_message($message,false, true);
		}
	}
	...
	{
		Display::display_normal_message(get_lang("OnlyAdminDeletePageWiki"), false, true);
	}
...
if ($_GET['action']=='recentchanges'){
	...
	if ( api_is_allowed_to_session_edit(false,true) ) {
		...
		{
			$notify_all= Display::return_icon('pixel.gif',get_lang('FullNotifyByEmail'),array('class'=>'actionplaceholderminiicon actionsendmail_checked','alt'=>get_lang('FullNotifyByEmail'),'style'=>'vertical-align:middle')).get_lang('NotNotifyChanges');
			$lock_unlock_notify_all='unlocknotifyall';
		}
		else
		{
			$notify_all= Display::return_icon('pixel.gif',get_lang('FullCancelNotifyByEmail'),array('class'=>'actionplaceholderminiicon actionsendmail_16','alt'=>get_lang('FullCancelNotifyByEmail'),'style'=>'vertical-align:middle')). get_lang('NotifyChanges');
	...
	if (Database::num_rows($allpages) > 0)
	{
		...
		while ($obj = Database::fetch_object($allpages))
		{
			...
			$row[] = '<a href="'.api_get_self().'?cidReq='.$_course[id].'&action=showpage&title='.urlencode($obj->reflink).'&amp;page_id='.$obj->id.'&group_id='.Security::remove_XSS($_GET['group_id']).'">'.$obj->title.'</a>';
  ...
  $sql='SELECT * FROM '.$tbl_wiki.'WHERE page_id="'.Database::escape_string($page_id).'" AND '.$groupfilter.$condition_session.' ORDER BY id ASC';
	...
	if ($id!='')
	{
		...
		if($row['visibility_disc']==1 || api_is_allowed_to_edit(false,true) || api_is_platform_admin() || ($row['assignment']==2 && $row['visibility_disc']==0 && (api_get_user_id()==$row['user_id'])))
		{
			...
			{
				$notify_disc= Display::return_icon('pixel.gif',get_lang('NotifyDiscussByEmail'),array('class'=>'actionplaceholderminiicon actionsendmail_checked','alt'=>get_lang('NotifyDiscussByEmail')));
				$lock_unlock_notify_disc='unlocknotifydisc';
			}
			else
			{
				$notify_disc= Display::return_icon('pixel.gif',get_lang('CancelNotifyDiscussByEmail'),array('class'=>'actionplaceholderminiicon actionsendmail_16','alt'=>get_lang('CancelNotifyDiscussByEmail')));
