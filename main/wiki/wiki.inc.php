...
function save_new_wiki(&$page_id, &$reflink) {
    ...
    $_clean['title'] = Database::escape_string(Security::remove_XSS($_POST['title']));
    $_clean['content'] = Database::escape_string(Security::remove_XSS($_POST['content'], COURSEMANAGERLOWSECURITY));
...
function display_new_wiki_form() {
    ...
    if ($currentEditor == 'fckeditor') {
	   ...
	   <tr><td width="5px;" class="toolbar_style"><img src="../img/list_add_16.png" onclick="wiki_link();"></td><td width="5px" class="toolbar_style"><img src="../img/pasteword_icon.png" onclick="word();"></td><td width="5px;" class="toolbar_style"><img src="../img/link_icon.png" onclick="link();"></td><td width="5px;" class="toolbar_style"><img src="../img/unlink_icon.png" onclick="unlink();"></td><td width="5px;" class="toolbar_style"><img src="../img/images_icon.png" onclick="image();"></td><td width="5px;" class="toolbar_style"><img src="../img/images_icon.gif" onclick="imagemap();"><td width="5px;" class="toolbar_style"><img src="../img/mindmap_18.png" onclick="mindmap();"></td><td width="5px;" class="toolbar_style"><img src="../img/Youtube.png" onclick="youtube();" alt="' . get_lang('Youtube') . '" title="' . get_lang('Youtube') . '"></td><td width="5px;" class="toolbar_style"><img src="../img/mascot_icon.png" onclick="mascot();"></td><td width="5px;" class="toolbar_style"><img src="../img/flash.png" onclick="flash();"></td><td width="5px;" class="toolbar_style"><img src="../img/embedmovies.png" onclick="embedmovies();"></td><td width="5px;" class="toolbar_style"><img src="../img/flvPlayer.gif" onclick="flvplayer();"></td><td width="5px;" class="toolbar_style"><img src="../img/audio.png" onclick="audio();"></td>' . $glossary_plugin . '<td width="5px;" class="toolbar_style"><img src="../img/table.png" width="16" height="16" onclick="table();"></td><td width="5px;" class="toolbar_style"><img src="../img/unordered_list.png" onclick="unordered();"></td><td width="5px;" class="toolbar_style"><img src="../img/view_source.png" onclick="source();"></td><td width="5px;" class="toolbar_style"><img src="../img/text_bold.png" onclick="makeitbold();"></td><td width="5px;" class="toolbar_style"><img src="../img/text_italic.png" onclick="makeititalic();"></td><td width="5px;" class="toolbar_style"><img src="../img/text_under.png" onclick="underline();"></td><td width="5px;" class="toolbar_style"><img src="../img/text_left.png" onclick="alignleft();"></td><td width="5px;" class="toolbar_style"><img src="../img/text_center.png" onclick="aligncenter();"></td><td width="5px;" class="toolbar_style"><img src="../img/text_right.png" onclick="alignright();"></td><td width="5px;" class="toolbar_style"><img src="../img/fontcolor.jpg" onclick="fontcolor(event);"></td><td width="5px"><table width="100%"><tr><td><span style="color:#333333;">Font:</span><select name="font" onchange="fontsize()"><option></option><option value="smaller" style="font-size: smaller;">smaller</option><option value="larger" style="font-size: larger;">larger</option><option value="xx-small" style="font-size: xx-small;">xx-small</option><option value="x-small" style="font-size: x-small;">x-small</option><option value="small" style="font-size: small;">small</option><option value="medium" style="font-size: medium;">medium</option><option value="large" style="font-size: large;">large</option><option value="x-large" style="font-size: x-large;">x-large</option><option value="xx-large" style="font-size: x-large;">xx-large</option></select></td></tr></table></td></tr><tr height="5px"><td></td></tr></table><br>';
    }
    echo '<div id="wikititle">';
    echo '<span class="form_required">*</span> ' . get_lang(Title) . ': <input type="text" name="title" value="' . urldecode($_GET['title']) . '" size="40" class="focus">';
    ...
    echo '</div><br/><br/>';
    api_disp_html_area('content', ' ', '', '', null, api_is_allowed_to_edit(null, true) ? array('ToolbarSet' => 'TestProposedAnswer', 'Width' => '100%', 'Height' => '500') : array('ToolbarSet' => 'TestProposedAnswer', 'Width' => '100%', 'Height' => '500', 'UserStatus' => 'student')
    );
    echo '<br/>';
    echo '<br/>';
...
function display_wiki_entry($newtitle, &$my_progress, &$count_my_words, &$wiki_score) {
    ...
    $sql = 'SELECT * FROM ' . $tbl_wiki . ', ' . $tbl_wiki_conf . ' WHERE ' . $tbl_wiki_conf . '.page_id=' . $tbl_wiki . '.page_id AND ' . $tbl_wiki . '.page_id="' . $page_id . '" AND ' . $tbl_wiki . '.' . $groupfilter . ' ' . $filter . ' ORDER BY id DESC';
    $result = Database::query($sql, __LINE__, __FILE__);
    ...
    if ($row['content'] == '' AND $row['title'] == '' AND $page == 'index') {
        if (api_is_allowed_to_edit(false, true) || api_is_platform_admin() || GroupManager :: is_user_in_group(api_get_user_id(), $_SESSION['_gid'])) {
            ...
            $default_table_for_content_Start = '<table align="center" width="98%" border="0"><tr><td align="left" width="40%" valign="top">';
    ...
    if ($KeyVisibility == "1" || api_is_allowed_to_edit(false, true) || api_is_platform_admin() || ($row['assignment'] == 2 && $KeyVisibility == "0" && (api_get_user_id() == $row['user_id']))) {
        ...
        /* 	echo '<span style="float:right; cursor: pointer;">';
          ...
          if (wiki_exist($title))
          {
          echo $icon_assignment.'&nbsp;'.$icon_task.'&nbsp;'.$title;
          }
          else
          {
          echo $title;
        ...
        /* echo '<div class="actions">';
          ...
          //menu discuss page
          echo '<a href="index.php?cidReq='.Security::remove_XSS($_GET['cidReq']).'&page_id='.$page_id.'&action=discuss&amp;title='.urlencode($page).'"'.is_active_navigation_tab('discuss').'>'.Display::return_icon('comment_bubble.gif',get_lang('DiscussThisPage')).' '.get_lang('Discuss').'</a>';
          }

          //menu history
          echo '<a href="index.php?cidReq='.Security::remove_XSS($_GET['cidReq']).'&page_id='.$page_id.'&action=history&amp;title='.urlencode($page).'&group_id='.$_clean['group_id'].'"'.is_active_navigation_tab('history').'>'.Display::return_icon('history.gif',get_lang('ShowPageHistory')).' '.get_lang('History').'</a>';
          //menu linkspages
          ...
          if(api_is_allowed_to_edit(false,true) || api_is_platform_admin()){
          echo '<a href="index.php?cidReq='.Security::remove_XSS($_GET['cidReq']).'&page_id='.$page_id.'&action=delete&amp;title='.urlencode($page).'"'.is_active_navigation_tab('delete').'>'.Display::return_icon('delete_link.png',get_lang('DeleteThisPage')).' '.get_lang('Delete').'</a>';
          }
          }
          echo '%&nbsp;&nbsp;&nbsp;'.get_lang('Progress').': '.$row['progress'].'%&nbsp;&nbsp;&nbsp;'.get_lang('Rating').': '.$row['score'].'&nbsp;&nbsp;&nbsp;'.get_lang('Words').': '.word_count($content);
          echo '</div>'; */
...
function display_wiki_search_results($search_term, $search_content=0) {
    ...
    if (Database::num_rows($result) > 0) {
        ...
        while ($obj = Database::fetch_object($result)) {
            ...
            if (api_is_allowed_to_edit(false, true) || api_is_platform_admin()) {
                $showdelete = ' <a href="' . api_get_self() . '?cidReq=' . $_course[id] . '&action=delete&page_id=' . Security::remove_XSS($obj->page_id) . '&title=' . urlencode(Security::remove_XSS($obj->reflink)) . '&group_id=' . Security::remove_XSS($_GET['group_id']) . '"><img src="../img/delete.png" title="' . get_lang('Delete') . '" alt="' . get_lang('Delete') . '" />';
            }
...
function display_back_for_more_page() {
    echo '<a href="index.php?action=more&amp;title=' . Security::Remove_XSS($_GET['title']) . '">';
    Display::display_icon('pixel.gif', get_lang('Back'),array('class'=>'toolactionplaceholdericon toolactionback')) . ' ' . get_lang('Back');
    echo '</a>';
}
?>