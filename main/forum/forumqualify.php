...
if ($origin=='learnpath') {
	...
	if (!empty($_SESSION['toolgroup'])) {
		...
		Display :: display_tool_header('');
        echo '<div class="actions">';
	    echo '<a href="viewthread.php?forum='.Security::remove_XSS($_GET['forum']).'&amp;gradebook='.$gradebook.'&amp;thread='.Security::remove_XSS($_GET['thread']).'">'.Display::return_icon('pixel.gif',get_lang('BackToThread'),array('class'=>'toolactionplaceholdericon toolactionback')).' '.get_lang('BackToThread').'</a>';
        echo '</div>';
