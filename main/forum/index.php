...
if (isset($_GET['action']) && $_GET['action'] == 'notify' AND isset($_GET['content']) AND isset($_GET['id'])) {
	...
	*/
	...
	echo '<div class="actions">';
        echo '<a href="index.php">'.Display::return_icon('pixel.gif',get_lang('BackToForumOverview'), array('class' => 'toolactionplaceholdericon toolactionback')).' '.get_lang('BackToForumOverview').'</a>';
		...
		foreach ($forum_categories_list as $forum_category_key => $forum_category) {
			...
			if((!isset($_SESSION['id_session']) || $_SESSION['id_session']==0) && !empty($forum_category['session_name'])) {
				...
				$session_displayed = '';
                        ...
                        echo'<table class="'.$parent_draggable.$draggable.' rounded move1" width="100%"><tr><td width="70%">';
