...
function show_add_forumcategory_form($inputvalues=array()) {
	...
	$form->addElement('text', 'forum_category_title', get_lang('Title'),'class="focus" style="width:365px;"');
	...
	$form->addElement('textarea','forum_category_comment',get_lang('Comment'),array ('rows' => '5', 'cols' => '50'));
...
function show_add_forum_form($inputvalues=array()) {
	...
	$form->addElement('text', 'forum_title', get_lang('Title'),'class="focus" style="width:300px;"');
	...
	$form->addElement('textarea','forum_comment',get_lang('Comment'),array ('rows' => '6', 'cols' => '40'));
...
function store_forumcategory($values) {
	...
	if (isset($values['forum_category_id'])) { // storing an edit
        ...
        $return_message.= ':'.'<a href="index.php?'.api_get_cidreq().'">'.get_lang('BackToForum').'</a>';
	    echo '<div class="confirmation-message rounded">'.$return_message.'</div>';
	} else {
...
function display_up_down_icon($content, $id, $list) {
	...
	if ($position>1) {
		$return_value='<td width="5%" align="center"><a href="'.api_get_self().'?'.api_get_cidreq().'&action=move&amp;direction=up&amp;content='.$content.'&amp;forumcategory='.Security::remove_XSS($_GET['forumcategory']).'&amp;id='.$id.'" title="'.get_lang('MoveUp').'">'.Display::return_icon('up.gif',get_lang('MoveUp')).'</a></td>';
	} else {
		$return_value=Display::return_icon('up_na.gif','-');
	}

	if ($position<$total_items) {
		$return_value.='<td width="5%" align="center"><a href="'.api_get_self().'?'.api_get_cidreq().'&action=move&amp;direction=down&amp;content='.$content.'&amp;forumcategory='.Security::remove_XSS($_GET['forumcategory']).'&amp;id='.$id.'" title="'.get_lang('MoveDown').'" >'.Display::return_icon('down.gif',get_lang('MoveDown')).'</a></td>';
	} else {

	   $return_value.= '<td width="5%" align="center">'.Display::return_icon('down_na.gif','-').'</td>';
	}
	echo $return_value;
}
...
function get_threads($forum_id) {
	...
	$sql = "SELECT thread.*, item_properties.*, users.firstname, users.lastname, users.user_id,
...
function store_thread($values) {
	...
	if($upload_ok) {
		...
		if ($has_attachment) {
			...
			if (!filter_extension($new_file_name))  {
				echo get_lang('UplUnableToSaveFileFilteredExtension');
			} else {
				if ($result) {
		...
		echo get_lang('UplNoFileUploaded');
	}
}
/**
...
function show_add_post_form($action='', $id='', $form_values='') {
	...
	$renderer->setElementTemplate('<div class="row"><div style="width:100%;float:left;">'.get_lang('Title').': {element}</div></div>', 'post_title');
...
//		$form->addElement('html','</div>');
	...
	if ($forum_setting['allow_sticky'] AND api_is_allowed_to_edit(null,true) AND $action=='newthread') {
...
function store_reply($values) {
	...
	if ($upload_ok) {
		// anonymous or not: if anonymous then we store the poster name that the user has entered
		if (api_is_anonymous() OR $_user['user_id'] == 0 OR empty($_user['user_id'])){
		...
		$sql="INSERT INTO $table_posts (post_title, post_text, thread_id, forum_id, poster_id, poster_name, post_date, post_notification, post_parent_id, visible)
				VALUES ('".Database::escape_string(Security::remove_XSS($values['post_title']))."',
						...
						'".Database::escape_string($values['forum_id'])."',
						'".Database::escape_string($_user['user_id'])."',
						'".Database::escape_string($poster_name)."',
						'".Database::escape_string($post_date)."',
		...
		echo get_lang('UplNoFileUploaded')." ". get_lang('UplSelectFileFirst');
	}
...
function show_edit_post_form($current_post, $current_thread, $current_forum, $form_values='',$id_attach=0) {
	...
	if ($forum_setting['allow_sticky'] and api_is_allowed_to_edit(null,true) and $current_post['post_parent_id']==0) { // the sticky checkbox only appears when it is the first post of a thread
		$form->addElement('checkbox', 'thread_sticky', '', get_lang('StickyPost'));
...
function display_user_image($user_id,$name, $origin='') {
	...
	if ($user_id<>0) {
		...
		$friends_profile = UserManager::get_picture_user($user_id, $image_path['file'], 0, 'medium_');
        if ($origin != 'learnpath') $str_link = $link;
        $str_link.='<img src="'.$friends_profile['file'].'" '.$friends_profile['style'].' alt="'.$name.'"  title="'.$name.'" />';
		if ($origin != 'learnpath') $str_link.='</a>';
        return $str_link;
	} else {
      if ($origin != 'learnpath') $str_link = $link;
      $str_link.='<img src="'.api_get_path(WEB_CODE_PATH)."img/unknown.jpg".'" alt="'.$name.'"  title="'.$name.'"  />';
	  if ($origin != 'learnpath') $str_link.='</a>';
      return $str_link;
	}
...
function get_whats_new() {
	...
	// to determine if we need to reload the the whatsnew_post_info we compare the last date that something is post with the one
	// that is stored in the session. The reason for this is that we want to know if somebody has added something new since the last time the whatsnew_post_info 
	// information has been retrieved from the database (and this query is a lot faster than always (unconditionally) getting the whatsnew_post_info information
	$sql="SELECT * FROM ".$table_posts." WHERE poster_id <> ".$_user['user_id']." ORDER BY post_id DESC";
	...
	// checking if we need to retrieve the information of the last time the user visited the forum
	...
	// getting all the information that is new for the user
	if (!$_SESSION['whatsnew_post_info']) {
		if ($_SESSION['last_forum_access']<>'') {
			$whatsnew_post_info = array();
			$sql="SELECT * FROM ".$table_posts." WHERE post_date>'".Database::escape_string($_SESSION['last_forum_access'])."' AND poster_id <> ".$_user['user_id'].""; // note: check the performance of this query.
			$result=Database::query($sql,__FILE__,__LINE__);
			while ($row=Database::fetch_array($result)) {
				// if the what's new information has been reloaded (because somebody else has posted a message while we are watching other new messages) 
				// then we only have to add to the what's new info if it has not been seen yet. 
				if ($whats_new_reloaded){
					if ($_SESSION['whatsnew_post_viewed'][$row['forum_id']][$row['thread_id']][$row['post_id']] == false){
						$whatsnew_post_info[$row['forum_id']][$row['thread_id']][$row['post_id']]=$row['post_date'];
					}
				}
				// the what's new information has not been reloaded, so we add everyhting that is new to $whatsnew_post_info 
				else{ 
					$whatsnew_post_info[$row['forum_id']][$row['thread_id']][$row['post_id']]=$row['post_date'];
				}
			}
...
function move_thread_form() {
	...
	$gradebook=Security::remove_XSS($_GET['gradebook']);
	// initiate the object
	$form = new FormValidator('movepost', 'post', api_get_self().'?forum='.Security::remove_XSS($_GET['forum']).'&gradebook='.$gradebook.'&thread='.Security::remove_XSS($_GET['thread']).'&action='.Security::remove_XSS($_GET['action']).'&origin='.$origin, null, array('class' => 'outer_form'));
	// the header for the form
...
function move_post_form() {
	...
	$gradebook=Security::remove_XSS($_GET['gradebook']);
	// initiate the object
	$form = new FormValidator('movepost', 'post', api_get_self().'?forum='.Security::remove_XSS($_GET['forum']).'&thread='.Security::remove_XSS($_GET['thread']).'&origin='.$origin.'&gradebook='.$gradebook.'&post='.Security::remove_XSS($_GET['post']).'&action='.Security::remove_XSS($_GET['action']).'&post='.Security::remove_XSS($_GET['post']), null, array('class' => 'outer_form'));
	// the header for the form
...
function forum_search() {
	global $origin;
	// initiate the object
	$form = new FormValidator('forumsearch','post','forumsearch.php?'.api_get_cidreq().'&origin='.$origin.'');
...
function display_forum_search_results($search_term) {
	...
	foreach ($search_terms as $key => $value) {
		$search_restriction[] = "(forum.forum_title like '%".Database::escape_string(trim($value))."%' OR forum.forum_comment like '%".Database::escape_string(trim($value))."%' OR posts.post_title LIKE '%".Database::escape_string(trim($value))."%'
									OR posts.post_text LIKE '%".Database::escape_string(trim($value))."%')";
	}

	$sql = "SELECT cat.cat_id,cat.cat_title,cat.cat_comment,forum.forum_id,forum.forum_title,forum.forum_comment,posts.post_id,posts.post_title,posts.post_text,posts.thread_id,item_properties.visibility,posts.visible FROM $table_categories cat LEFT JOIN $table_forums forum ON cat.cat_id=forum.forum_category LEFT JOIN  $table_posts posts ON forum.forum_id = posts.forum_id
			INNER JOIN $tbl_item_property item_properties ON item_properties.ref=forum.forum_id	WHERE (".implode(' AND ',$search_restriction)." AND item_properties.visibility=1 AND item_properties.tool='".TOOL_FORUM."') ";
	...
	while ($row = Database::fetch_array($result,'ASSOC')) {
		...
		/*
			...
			4. post is visible
		*/
        $thread_list = get_thread_information($row['thread_id']);
        // If no is administrator
		if (!api_is_allowed_to_edit(null,true)) {
            // Checks if the post is visible
            if (empty($row['post_id'])) {
              $post_is_visible = true;
            } else {
              $check_visible = $row['post_id'] > 0 ? $row['visible'] : 0;
              $post_is_visible = ($check_visible == 1) ? true : false;
            }
            // Checks if the thread is visible
            $thread_is_visible = false;
            if (empty($row['thread_id'])) {
              $thread_is_visible = true;
            } else {
              if ($thread_list['visibility'] == '1') {
                $thread_is_visible = true;
              }
            }

			if ($forum_categories_list[$row['cat_id']]['visibility'] == '1'  && $forum_list[$row['forum_id']]['visibility'] == '1' && $thread_is_visible && $post_is_visible) {
				$display_result = true;
			}
		} else {
			$display_result = true;
		...
		if ($display_result == true) {
			$search_results_item = '<li><a href="viewforumcategory.php?forumcategory='.$row['cat_id'].'&origin='.$origin.'&amp;search='.urlencode($search_term).'">'.$row['cat_title'].'</a> > ';
			$search_results_item .= '<a href="viewforum.php?forum='.$row['forum_id'].'&amp;origin='.$origin.'&amp;search='.urlencode($search_term).'">'.$row['forum_title'].'</a> > ';
		 		...
		 		foreach($threads as $thread) {
		 				if ($i<=4) {
			 				...
			 				if (is_array($post_list) && count($post_list)>0) {
				 				...
				 				$hand_forums.= Display::return_icon('pixel.gif',get_lang('forumthread'),array('class'=>'actionplaceholdericon actionforumthread'));
...
/**
 * Return HTML code to display columns titles in forums list
 * called only if there are forums in group
 * 
 * @since oct 2010
 * @return string
 */
